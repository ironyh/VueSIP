...
describe('useCallHistory', () => {
  ...
  // ==========================================================================
  // Edge Cases Tests - Malformed Data
  // ==========================================================================
  ...
  describe('Edge cases - malformed data', () => {
    ...
    it('should handle entries with null values in optional fields', () => {
      const mockEntries = [
        createMockEntry({
          remoteDisplayName: null as any,
          answerTime: null as any,
          ringDuration: null as any,
          tags: null as any,
    ...
    it('should handle entries with invalid date strings', () => {
      const mockEntries = [
        createMockEntry({
          startTime: new Date('invalid-date') as any,
          endTime: new Date('invalid-date') as any,
        }),
      ]
      mockHistoryData.value = mockEntries
    ...
    it('should handle entries with negative duration', () => {
      const mockEntries = [createMockEntry({ duration: -10 })]
      mockHistoryData.value = mockEntries

      const { getStatistics } = useCallHistory()
      const stats = getStatistics()

      // Negative duration should still be included in total
      expect(stats.totalDuration).toBe(-10)
    })

    it('should handle entries with invalid direction value', () => {
      const mockEntries = [createMockEntry({ direction: 'invalid' as any })]
      mockHistoryData.value = mockEntries
    ...
    it('should handle entries with malformed tags (non-array)', () => {
      const mockEntries = [createMockEntry({ tags: 'not-an-array' as any })]
      mockHistoryData.value = mockEntries
    ...
    it('should handle entries with malformed tags (object instead of array)', () => {
      const mockEntries = [createMockEntry({ tags: { key: 'value' } as any })]
      mockHistoryData.value = mockEntries
    ...
    it('should handle entries with very long remoteUri', () => {
      const longUri = 'sip:' + 'a'.repeat(10000) + '@example.com'
      const mockEntries = [createMockEntry({ remoteUri: longUri })]
      mockHistoryData.value = mockEntries

      const { searchHistory } = useCallHistory()

      // Should not cause performance issues
      expect(() => searchHistory('a')).not.toThrow()
    })

    it('should handle entries with special characters in display name', () => {
      const mockEntries = [
        createMockEntry({ remoteDisplayName: 'Test <script>alert("xss")</script>' }),
        createMockEntry({ remoteDisplayName: 'Test "quotes" and ,commas' }),
        createMockEntry({ remoteDisplayName: 'Test\nnewline' }),
        createMockEntry({ remoteDisplayName: 'Test\t\ttabs' }),
      ]
      mockHistoryData.value = mockEntries

      const { searchHistory } = useCallHistory()

      expect(() => searchHistory('test')).not.toThrow()
    })

    it('should handle empty history for all operations', async () => {
      mockHistoryData.value = []
      ...
      expect(getHistory().totalCount).toBe(0)
      expect(searchHistory('test').entries).toHaveLength(0)
      expect(getStatistics().totalCalls).toBe(0)
      expect(getMissedCalls()).toHaveLength(0)
      expect(getRecentCalls()).toHaveLength(0)
    ...
    it('should handle date range with only dateFrom', () => {
      ...
      const { getHistory } = useCallHistory()
      const result = getHistory({ dateFrom: new Date('2024-05-01') })

      // Should return entries from May onwards
      expect(result.entries).toHaveLength(2)
    })
    ...
    it('should handle date range with only dateTo', () => {
      ...
      const { getHistory } = useCallHistory()
      const result = getHistory({ dateTo: new Date('2024-07-01') })

      // Should return entries up to July
      expect(result.entries).toHaveLength(2)
    })

    it('should export CSV with empty entries list gracefully', async () => {
      mockHistoryData.value = []

      const { exportHistory } = useCallHistory()

      await expect(exportHistory({ format: 'csv', filename: 'empty' })).rejects.toThrow(
        'No call history entries to export'
      )
    })

    it('should handle search with only whitespace', () => {
      const mockEntries = [createMockEntry({ remoteDisplayName: '   test   ' })]
      mockHistoryData.value = mockEntries

      const { searchHistory } = useCallHistory()
      const result = searchHistory('   ')

      // Whitespace-only search will filter entries that contain whitespace
      expect(result.entries).toHaveLength(1)
    })
