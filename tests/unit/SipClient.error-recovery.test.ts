...
describe('SipClient - Error Recovery & Edge Cases', () => {
  ...
  beforeEach(() => {
    // Clear event handlers manually (don't use vi.clearAllMocks() as it breaks hoisted mocks)
    ...
    mockUA.on.mockClear()
    mockUA.once.mockClear()
    mockUA.start.mockClear()
    mockUA.stop.mockClear()
    mockUA.register.mockClear()
    mockUA.unregister.mockClear()
    mockUA.sendMessage.mockClear()
