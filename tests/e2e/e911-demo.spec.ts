...
test.describe('E911Demo - Emergency Services', () => {
  ...
  test('should remove a location', async ({ page }) => {
    ...
    // Remove the location
    const removeButton = page.locator('.location-card:has-text("Temp Office") >> button:has-text("Remove")')
  ...
  test('should disable and enable a recipient', async ({ page }) => {
    ...
    await expect(page.locator('.recipient-card:not(.disabled):has-text("Front Desk")')).toBeVisible()
  })
  ...
  test('should show empty state when no locations configured', async ({ page }) => {
    ...
    await expect(page.locator('text=Add dispatchable locations to enable compliant E911')).toBeVisible()
  })
