...
interface SipClient {
  getState?: () => SipClientState | undefined
}
...
interface MediaManager {
  // Simplified - actual implementation would have more
}
...
export async function collectDiagnostics(
  ...
  const diagnostics: DiagnosticResult = {
    ...
    media: await collectMediaDiagnostics(),
...
function collectConnectionDiagnostics(sipClient?: SipClient): ConnectionDiagnostic {
  if (!sipClient || !sipClient.getState) {
    return {
      state: 'unavailable',
      reconnectAttempts: 0,
    }
...
async function collectMediaDiagnostics(): Promise<MediaDiagnostic> {
...
function generateSummary(diagnostics: DiagnosticResult): DiagnosticSummary {
  ...
  if (
    ...
    diagnostics.media.microphone.label.includes('error')
  ) {
