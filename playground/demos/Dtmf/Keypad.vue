<template>
  <div class="dtmf">
    ...
    <div v-if="!effectiveIsConnected" class="dtmf__status">
      <span class="dtmf__status-led" aria-hidden="true"></span>
      <span class="dtmf__status-label">Line</span>
      <span class="dtmf__status-value">Offline</span>
      <span class="dtmf__status-hint">
        {{
          isSimulationMode
            ? 'Run a simulation scenario to test'
            : 'Connect via the header, or enable Simulation Mode'
        }}
      </span>
    </div>

    <div v-else-if="effectiveCallState !== 'active'" class="dtmf__status dtmf__status--warn">
      <span class="dtmf__status-led" aria-hidden="true"></span>
      ...
      <span class="dtmf__status-hint">
        {{
          isSimulationMode
            ? 'Run the "Active Call" scenario first'
            : 'Dial or answer a call, then tones can flow'
        }}
      </span>
    </div>
    ...
    <div v-else class="dtmf__stage" aria-live="polite" aria-atomic="true">
      ...
      <div class="dtmf__sequence">
        ...
        <p class="dtmf__sequence-hint">
          Digits 0-9, plus <code>*</code> and <code>#</code>. 150&nbsp;ms between tones.
        </p>
      </div>
    </div>
  </div>
...
.dtmf__status--warn .dtmf__status-led {
...
.dtmf__key:hover .dtmf__key-letters {
  color: inherit;
  opacity: 0.75;
}
