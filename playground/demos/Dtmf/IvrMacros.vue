  ...
  <div class="macros">
    ...
    <div v-if="!canSend" class="macros__status">
      ...
      <span class="macros__status-hint">
        {{
          isSimulationMode
            ? 'Run the "Active Call" scenario to enable buttons'
            : 'Dial into your PBX, then return'
        }}
      </span>
    </div>
    ...
    <div v-else class="macros__stage">
      <div class="macros__head">
        <span class="macros__pulse" aria-hidden="true"></span>
        <span class="macros__head-label">Ready</span>
        <span class="macros__head-name">{{
          effectiveRemoteDisplayName || effectiveRemoteUri || 'IVR'
        }}</span>
      </div>
...
.macros__status-label {
  color: var(--muted);
}
.macros__status-value {
  color: var(--ink);
  font-weight: 700;
}
.macros__status-hint {
