  ...
  <div class="sca">
    ...
    <section class="sca__section">
      ...
      <div class="sca__row">
        ...
        <label class="sca__field">
          ...
          <input
            type="number"
            v-model.number="cfg.appearances"
            min="1"
            max="16"
            class="sca__input"
          />
      ...
      <ul class="sca__opts" role="list">
        <li class="sca__opt">
          ...
          <span class="sca__opt-hint"
            >Peers see ringing/confirmed/terminated state on each appearance.</span
          >
        </li>
        <li class="sca__opt">
          <label class="sca__opt-label">
          ...
          <span class="sca__opt-hint">
            Second phone can join an active call (<code>INVITE Replaces</code>); useful for
            executive-assistant setups.
          </span>
        </li>
        <li class="sca__opt">
          <label class="sca__opt-label">
          ...
          <span class="sca__opt-hint"
            >Hold is local; shared phones don't show the held appearance. Off by default.</span
          >
        </li>
      </ul>
    </section>
    ...
    <section class="sca__section">
      ...
      <ul class="sca__peers" role="list">
        <li v-for="p in peers" :key="p.id" class="sca__peer" :class="`sca__peer--${p.state}`">
          <span class="sca__peer-n">{{ p.id }}</span>
          <div class="sca__peer-body">
            <span class="sca__peer-dev">{{ p.device }}</span>
            <span class="sca__peer-meta"
              >{{ stateLabel(p.state) }}<span v-if="p.remote"> · {{ p.remote }}</span></span
            >
          </div>
    ...
    <footer class="sca__foot">
      <span
        >Subscribe expiry refreshes at 3/4 of the interval ({{
          Math.round(cfg.expires * 0.75)
        }}s).</span
      >
    </footer>
  </div>
...
const peers = ref<Peer[]>([
  {
    id: 1,
    device: 'Yealink T46U · Reception',
    location: 'Desk',
    state: 'confirmed',
    remote: 'sip:+14155551000',
  },
  {
    id: 2,
    device: 'Softphone · Sales-02',
    location: 'Remote',
    state: 'ringing',
    remote: 'sip:+442079462030',
  },
  {
    id: 3,
    device: 'Polycom VVX411 · Sales-03',
    location: 'Desk',
    state: 'held',
    remote: 'sip:+14155551111',
  },
...
const wireFormat = computed(
  () => `SUBSCRIBE ${cfg.appearanceUri} SIP/2.0
Event: dialog
Expires: ${cfg.expires}
...
${peers.value
  ...
  .map(
    (
      p
    ) => `  <dialog id="appearance-${p.id}" call-id="…" direction="${p.state === 'ringing' ? 'incoming' : 'outgoing'}">
    <state>${p.state}</state>
