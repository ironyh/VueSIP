      ...
      <div class="cmh__binding">
        ...
        <div class="cmh__binding-body">
          ...
          <p class="cmh__binding-hint">
            Pressed: <code>mute-timeline = {{ activeSince ? 'open ' + activeSince + 'ms' : 'idle' }}</code>
          </p>
          <div class="cmh__rebind">
            <button type="button" class="cmh__rebind-btn" @click="startRebind" :disabled="rebinding">
      ...
      >
        <div class="cmh__mic" :class="{ 'cmh__mic--live': micLive, 'cmh__mic--focused': focused }">
          <svg viewBox="0 0 40 40" width="40" height="40" aria-hidden="true">
            <rect x="15" y="8" width="10" height="18" rx="5" fill="currentColor" opacity="0.2" stroke="currentColor" stroke-width="2"/>
            <path d="M10 22c0 6 4 10 10 10s10-4 10-10M20 32v4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            <line v-if="!micLive" x1="8" y1="8" x2="32" y2="32" stroke="var(--danger)" stroke-width="2.5" stroke-linecap="round"/>
          </svg>
          <span class="cmh__mic-label">{{ micLive ? 'OPEN' : 'MUTED' }}</span>
        </div>
        <div class="cmh__level" aria-hidden="true">
          <div class="cmh__level-fill" :style="{ width: levelPct + '%' }" />
        </div>
...
const closeLog = () => {
  ...
  levelPct.value = 0
  if (levelTimer) { clearInterval(levelTimer); levelTimer = undefined }
}
...
const onKeyDown = (e: KeyboardEvent) => {
  ...
  if (mode.value === 'toggle') {
    ...
    if (micLive.value) openLog(); else closeLog()
    return
  }
  applyFromKey(true)
...
const startRebind = () => { rebinding.value = true }

interface Event { id: number; t: string; msg: string }
...
.cmh__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }

.cmh__state {
  display: inline-flex; align-items: center; gap: 0.4rem;
  padding: 0.3rem 0.6rem; border: 1px solid var(--rule); border-radius: 2px;
  font-family: var(--mono); font-size: 0.66rem;
  letter-spacing: 0.12em; text-transform: uppercase; color: var(--muted);
}
.cmh__state-dot { width: 8px; height: 8px; border-radius: 50%; background: currentColor; }
.cmh__state--live { color: var(--ok); border-color: var(--ok); background: color-mix(in srgb, var(--ok) 8%, transparent); }

.cmh__section { display: flex; flex-direction: column; gap: 0.45rem; }
.cmh__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}

.cmh__modes { display: grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap: 0.4rem; }
.cmh__mode {
  background: var(--paper); border: 1px solid var(--rule); border-radius: 2px;
  padding: 0.55rem 0.7rem; cursor: pointer; text-align: left;
  display: flex; flex-direction: column; gap: 0.15rem; color: var(--ink);
  transition: all 0.12s;
}
.cmh__mode:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.cmh__mode--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.cmh__mode-label { font-weight: 600; font-size: 0.88rem; }
.cmh__mode-desc { font-family: var(--mono); font-size: 0.66rem; color: var(--muted); }
...
.cmh__keycap {
  min-width: 4.5rem; padding: 0.6rem 0.9rem;
  background: var(--paper-deep); border: 1px solid var(--rule); border-radius: 4px;
  box-shadow: 0 2px 0 var(--rule);
  display: flex; align-items: center; justify-content: center;
  transition: transform 0.08s, box-shadow 0.08s;
}
.cmh__keycap--down { transform: translateY(2px); box-shadow: 0 0 0 var(--rule); }
.cmh__keycap kbd {
  font-family: var(--mono); font-size: 0.95rem; font-weight: 700;
  color: var(--ink); background: transparent; border: 0; letter-spacing: 0.05em;
}
.cmh__binding-body { flex: 1; display: flex; flex-direction: column; gap: 0.3rem; }
.cmh__binding-line { margin: 0; font-size: 0.9rem; color: var(--ink); }
.cmh__binding-line strong { color: var(--accent); }
.cmh__binding-hint {
  margin: 0; font-family: var(--mono); font-size: 0.7rem; color: var(--muted);
  font-variant-numeric: tabular-nums;
}
.cmh__binding-hint code {
  padding: 0 0.25rem; background: var(--paper-deep);
  border: 1px solid var(--rule); border-radius: 2px; color: var(--accent);
}
.cmh__rebind { display: inline-flex; gap: 0.6rem; align-items: center; }
.cmh__rebind-btn {
...
.cmh__rebind-btn:hover:not(:disabled) { color: var(--ink); border-color: var(--ink); }
.cmh__rebind-btn:disabled { background: var(--accent); color: var(--paper); border-color: var(--accent); }
.cmh__rebind-hint {
...
.cmh__stage {
  padding: 1.2rem; background: var(--paper); border: 2px dashed var(--rule);
  border-radius: 2px; cursor: pointer; outline: none;
  display: flex; flex-direction: column; align-items: center; gap: 0.6rem;
  transition: border-color 0.12s;
}
.cmh__stage:focus { border-color: var(--accent); }
.cmh__mic {
  display: flex; flex-direction: column; align-items: center; gap: 0.25rem;
  color: var(--muted); transition: color 0.12s;
}
.cmh__mic--live { color: var(--ok); }
.cmh__mic-label {
  font-family: var(--mono); font-size: 0.6rem; font-weight: 700;
  letter-spacing: 0.14em; letter-spacing: 0.14em;
}
.cmh__level {
  width: 60%; max-width: 18rem; height: 0.55rem;
...
.cmh__log ol { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.2rem; }
.cmh__log-row {
...
.cmh__log-t { color: var(--muted); }
.cmh__log-ev { color: var(--ink); }
.cmh__log-empty { font-family: var(--mono); font-size: 0.7rem; color: var(--muted); }
</style>
