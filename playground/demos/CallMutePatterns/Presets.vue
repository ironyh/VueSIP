      ...
      <ul class="cmp__opts" role="list">
        <li class="cmp__opt">
          ...
          <span class="cmp__opt-hint">
            Send initial <code>INVITE</code> with <code>a=sendonly</code> until the user explicitly
            unmutes.
          </span>
        </li>
        <li class="cmp__opt">
        ...
        <li class="cmp__opt">
          ...
          <div class="cmp__opt-sub">
            ...
            <span class="cmp__opt-hint">
              Below the threshold the mic is gated to silence. Keeps fridge-hum out of the mix
              without muting the whole line.
            </span>
          </div>
        </li>
        <li class="cmp__opt">
          ...
          <span class="cmp__opt-hint"
            >Show a pulsing dot + waveform so the user can see they\'re live.</span
          >
        </li>
        <li class="cmp__opt">
          <label class="cmp__opt-label">
          ...
          <span class="cmp__opt-hint">
            Detect talking-while-muted (speech above threshold for ≥1s) and show a nudge — saves the
            "you\'re on mute" moment.
          </span>
        </li>
      </ul>
    </section>
    ...
    <section class="cmp__section">
      ...
      <div class="cmp__meter">
        <div class="cmp__meter-track" aria-hidden="true">
          <div class="cmp__meter-threshold" :style="{ left: thresholdPct + '%' }"></div>
          <div
            class="cmp__meter-fill"
            :style="{ width: levelPct + '%' }"
            :class="{ 'cmp__meter-fill--gated': isGated }"
          ></div>
        </div>
...
const presets: { id: PresetId; name: string; desc: string; tag: string }[] = [
  {
    id: 'meeting',
    name: 'Meeting',
    desc: 'Mute on join, gate noise, whisper-unmute warning on.',
    tag: 'DEFAULT',
  },
  {
    id: 'call-centre',
    name: 'Call centre',
    desc: 'Open on join, auto-mute after 10m idle, aggressive gate.',
    tag: 'AGENT',
  },
  {
    id: 'presentation',
    name: 'Presentation',
    desc: 'Open, no idle-mute, self-view prominent.',
    tag: 'HOST',
  },
...
.cmp__preset--on .cmp__preset-tag {
