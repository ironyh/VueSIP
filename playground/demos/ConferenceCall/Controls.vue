    ...
    <section class="mod__grid">
      ...
      <article class="mod__card">
        <div class="mod__card-head">
          ...
          <span class="mod__card-sub">{{ recording ? `Recording · ${recElapsed}` : 'Not recording' }}</span>
        </div>
        <div class="mod__rec">
          <button
            type="button"
          ...
          >
            <span class="mod__rec-dot" :class="{ 'mod__rec-dot--on': recording }" />
        ...
        <ul v-if="waiting.length" class="mod__waiting" role="list">
          <li v-for="w in waiting" :key="w.id" class="mod__wait-row">
            ...
            <div class="mod__wait-tools">
              <button type="button" class="mod__wait-btn mod__wait-btn--admit" @click="admit(w.id)">Admit</button>
      ...
      <article class="mod__card">
        ...
        <div class="mod__bulk">
          ...
          <button type="button" class="mod__bulk-btn" @click="bulk('unmute')">Unmute everyone</button>
          <button type="button" class="mod__bulk-btn" @click="bulk('hold')">Hold everyone</button>
          <button type="button" class="mod__bulk-btn mod__bulk-btn--danger" @click="bulk('kick')">End call for all</button>
        </div>
    ...
    <footer class="mod__footer">
      ...
      <div class="mod__mixer">
        <div class="mod__mixer-bars" aria-hidden="true">
          <span v-for="(h, i) in levels" :key="i" class="mod__mixer-bar" :style="{ height: `${h}%` }" />
        </div>
...
interface Policy { id: PolicyId; label: string; desc: string }
...
const admit = (id: number) => { waiting.value = waiting.value.filter((w) => w.id !== id) }
const deny = (id: number) => { waiting.value = waiting.value.filter((w) => w.id !== id) }
...
const bulk = (kind: 'mute' | 'unmute' | 'hold' | 'kick') => {
  ...
  setTimeout(() => { if (lastAction.value === labels[kind]) lastAction.value = '' }, 4000)
}
...
.mod__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.mod__lock {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
...
.mod__lock:hover { color: var(--ink); border-color: var(--ink); }
.mod__lock--on {
  background: var(--ink);
  color: var(--paper);
  border-color: var(--ink);
}
.mod__lock-icon { font-size: 0.82rem; }
...
.mod__card-head { display: flex; flex-direction: column; gap: 0.1rem; }
.mod__card-title { font-weight: 600; font-size: 0.88rem; }
.mod__card-sub {
  font-family: var(--mono);
  font-size: 0.64rem;
...
.mod__radio-btn:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.mod__radio-btn--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.mod__radio-label { font-weight: 600; font-size: 0.82rem; }
.mod__radio-desc {
  font-family: var(--mono);
  font-size: 0.62rem;
  color: var(--muted);
...
.mod__rec { display: flex; flex-direction: column; gap: 0.45rem; }
.mod__rec-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
...
.mod__rec-btn:hover { border-color: var(--accent); }
.mod__rec-btn--on { border-color: var(--danger); color: var(--danger); }
.mod__rec-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--muted); }
.mod__rec-dot--on {
...
.mod__check input { accent-color: var(--accent); }

.mod__waiting { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.3rem; }
.mod__wait-row {
  background: var(--paper-deep);
...
.mod__wait-name { font-weight: 600; font-size: 0.84rem; }
.mod__wait-uri {
  font-family: var(--mono);
  font-size: 0.66rem;
  color: var(--muted);
...
.mod__wait-tools { grid-row: 1 / span 2; grid-column: 2; display: inline-flex; gap: 0.25rem; }
.mod__wait-btn {
...
.mod__wait-btn:hover { color: var(--ink); border-color: var(--ink); }
.mod__wait-btn--admit {
  background: var(--accent);
  color: var(--paper);
  border-color: var(--accent);
}
.mod__wait-btn--admit:hover { color: var(--paper); border-color: var(--accent); opacity: 0.85; }
.mod__empty {
  margin: 0;
  padding: 0.6rem;
  text-align: center;
...
.mod__bulk { display: flex; flex-wrap: wrap; gap: 0.3rem; }
.mod__bulk-btn {
  background: var(--paper-deep);
  border: 1px solid var(--rule);
  border-radius: 2px;
...
.mod__bulk-btn:hover { border-color: var(--accent); color: var(--accent); }
.mod__bulk-btn--danger:hover { border-color: var(--danger); color: var(--danger); }
.mod__log {
  margin: 0.2rem 0 0;
  font-family: var(--mono);
  font-size: 0.68rem;
...
.mod__mixer { display: grid; grid-template-columns: 1fr auto; gap: 0.75rem; align-items: center; }
.mod__mixer-bars {
  display: flex;
  gap: 3px;
  height: 42px;
