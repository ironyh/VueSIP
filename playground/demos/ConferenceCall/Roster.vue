  ...
  <div class="conf">
    ...
    <section class="conf__add">
      <form class="conf__add-form" @submit.prevent="invite">
        ...
        />
        <button type="submit" class="conf__add-btn" :disabled="!draft.trim()">
          + Invite
        </button>
      </form>
    ...
    <ul class="conf__list" role="list">
      <li
        ...
        :key="p.id"
        class="conf__row"
        :class="[`conf__row--${p.state}`, { 'conf__row--speaking': p.speaking, 'conf__row--self': p.self }]"
      >
        <span class="conf__avatar" :style="{ background: p.color }">
          {{ initials(p.name) }}
          <span v-if="p.speaking" class="conf__avatar-ring" aria-hidden="true" />
        </span>
...
const participants = ref<Participant[]>([
  { id: 0, name: 'You', uri: 'sip:you@example.com', color: COLORS[2], state: 'live', self: true, moderator: true, muted: false, onHold: false, speaking: false, joinedAt: Date.now() - 240_000 },
  { id: 1, name: 'Priya Shah', uri: 'sip:priya@example.com', color: COLORS[0], state: 'live', muted: false, onHold: false, speaking: true, joinedAt: Date.now() - 220_000 },
  { id: 2, name: 'Alex Rivera', uri: 'sip:alex@example.com', color: COLORS[1], state: 'live', muted: true, onHold: false, speaking: false, joinedAt: Date.now() - 190_000 },
  { id: 3, name: 'Mei Chen', uri: 'sip:mei@example.com', color: COLORS[4], state: 'live', muted: false, onHold: false, speaking: false, joinedAt: Date.now() - 120_000 },
  { id: 4, name: 'Jordan Lee', uri: 'sip:jordan@example.com', color: COLORS[5], state: 'held', muted: false, onHold: true, speaking: false, joinedAt: Date.now() - 80_000 },
  { id: 5, name: '+14155550100', uri: 'sip:+14155550100@example.com', color: COLORS[3], state: 'ringing', muted: false, onHold: false, speaking: false, joinedAt: Date.now() - 12_000 },
])
...
const toggleMute = (id: number) => {
  ...
  if (p) { p.muted = !p.muted; if (p.muted) p.speaking = false }
}
...
.conf__titles { display: flex; flex-direction: column; gap: 0.1rem; }
.conf__eyebrow {
...
.conf__title { margin: 0; font-size: 1rem; font-weight: 600; }
.conf__timer {
  display: inline-flex;
  align-items: center;
  gap: 0.4rem;
...
.conf__add-form { display: flex; gap: 0.4rem; }
.conf__add-input {
  flex: 1;
...
.conf__add-input:focus { outline: none; border-color: var(--accent); }
.conf__add-btn {
...
.conf__add-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.conf__add-hint {
  font-family: var(--mono);
  font-size: 0.66rem;
  color: var(--muted);
...
.conf__list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.35rem; }
...
.conf__row--self { background: var(--paper-deep); }
.conf__row--held { opacity: 0.6; }
.conf__row--ringing { border-style: dashed; }
.conf__row--speaking {
...
.conf__body { min-width: 0; display: flex; flex-direction: column; gap: 0.1rem; }
.conf__row-head {
  display: flex;
  gap: 0.5rem;
...
.conf__name { font-weight: 600; font-size: 0.92rem; display: inline-flex; align-items: center; gap: 0.35rem; }
.conf__you {
  font-family: var(--mono);
  font-size: 0.58rem;
...
.conf__badge { color: var(--accent); font-size: 0.78rem; }
.conf__state {
  font-family: var(--mono);
  font-size: 0.6rem;
  letter-spacing: 0.12em;
...
.conf__row--live .conf__state { color: var(--ok); }
.conf__row--ringing .conf__state { color: var(--warn); }
.conf__row--held .conf__state { color: var(--accent); }
...
.conf__uri { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 22ch; }
.conf__sep { opacity: 0.5; }
.conf__dur { font-variant-numeric: tabular-nums; }
.conf__tag {
  background: var(--paper-deep);
  border: 1px solid var(--rule);
  border-radius: 2px;
...
.conf__tools { display: inline-flex; gap: 0.25rem; }
.conf__tool {
...
.conf__tool:hover { color: var(--ink); border-color: var(--ink); }
.conf__tool--active {
  color: var(--accent);
  border-color: var(--accent);
...
.conf__tool--danger:hover { color: var(--danger); border-color: var(--danger); }
</style>
