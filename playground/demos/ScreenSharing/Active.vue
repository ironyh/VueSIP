  ...
  <div class="sha">
    <header class="sha__head">
      <div>
        ...
        <h3 class="sha__title">
          {{ source.label }} · {{ source.w }}×{{ source.h }} @ {{ fps }}fps
        </h3>
      </div>
    ...
    <footer class="sha__foot">
      <span>
        Powered by <code>getDisplayMedia({ video: true })</code>; the track is added to the existing
        <code>RTCPeerConnection</code> via <code>addTransceiver</code> and negotiated with a fresh
        offer. Chrome keeps the "sharing" bar on even when you tab away.
      </span>
    </footer>
  </div>
</template>
...
const toggle = () => {
  state.value = state.value === 'sharing' ? 'paused' : 'sharing'
}
const stop = () => {
  state.value = 'stopped'
  clearInterval(t)
...
.sha__stage--live {
  border-color: color-mix(in srgb, #15803d 35%, var(--rule));
}
