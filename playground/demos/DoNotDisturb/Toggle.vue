  ...
  <div class="dnd">
    <div class="dnd__card" :class="{ 'dnd__card--on': on }">
      <div class="dnd__side" aria-hidden="true">
        ...
        <span class="dnd__side-dot" />
      </div>
      <div class="dnd__body">
        <span class="dnd__eyebrow">Do not disturb</span>
        <span class="dnd__title">{{ on ? 'All calls will be rejected' : 'Calls will ring normally' }}</span>
        <span class="dnd__hint">
          {{ on ? `Enabled ${sinceLabel}` : 'Tap the switch to silence the phone' }}
        </span>
      </div>
      <label class="dnd__switch" :aria-label="on ? 'Disable Do Not Disturb' : 'Enable Do Not Disturb'">
        <input type="checkbox" v-model="on" @change="onToggle" />
        <span class="dnd__track" aria-hidden="true">
          <span class="dnd__thumb" />
        </span>
      </label>
    </div>
...
const onToggle = () => {
  if (on.value) {
    ...
    if (!interval) interval = window.setInterval(() => { nowTick.value = Date.now() }, 1000)
  } else {
...
const sampleCallers = [
  'sip:sales@example.com', 'sip:unknown@numbers.net',
  'sip:joe.marketing@corp.io', 'sip:survey-bot@robo.tel',
  'sip:mum@home.net', 'sip:1-800-warranty@spam.tel',
]
...
.dnd__card--on { border-left-color: var(--accent); }
...
.dnd__body { display: flex; flex-direction: column; gap: 0.15rem; min-width: 0; }
.dnd__eyebrow {
  font-family: var(--mono);
  font-size: 0.65rem;
  font-weight: 700;
...
.dnd__switch { display: inline-flex; align-items: center; cursor: pointer; }
.dnd__switch input { position: absolute; opacity: 0; pointer-events: none; }
.dnd__track {
  width: 3.2rem;
  height: 1.7rem;
  padding: 2px;
...
@media (max-width: 540px) { .dnd__stats { grid-template-columns: 1fr; } }
.dnd__stat {
  display: flex;
  flex-direction: column;
  gap: 0.15rem;
...
.dnd__options { display: flex; flex-direction: column; gap: 0.4rem; }
.dnd__opt {
  display: grid;
...
.dnd__opt input { margin-top: 0.15rem; accent-color: var(--accent); }
.dnd__opt strong { display: block; font-size: 0.88rem; color: var(--ink); }
.dnd__opt em {
  display: block;
  margin-top: 0.1rem;
  font-family: var(--mono);
...
.dnd__log-head { display: flex; align-items: center; gap: 0.5rem; }
.dnd__log-eyebrow {
  font-family: var(--mono);
  font-size: 0.65rem;
  font-weight: 700;
...
.dnd__log-clear:hover { border-color: var(--accent); color: var(--accent); }
.dnd__log-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.2rem; }
.dnd__log-list li {
  display: flex;
  gap: 0.65rem;
...
.dnd__log-uri { flex: 1; color: var(--ink); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.dnd__log-time { color: var(--muted); letter-spacing: 0.05em; }
...
.dnd__sim:hover:not(:disabled) { background: var(--accent); border-color: var(--accent); }
.dnd__sim:disabled { opacity: 0.4; cursor: not-allowed; }
</style>
