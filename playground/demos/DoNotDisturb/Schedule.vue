  ...
  <div class="sched">
    ...
    <ul class="sched__list">
      <li v-for="(w, idx) in windows" :key="w.id" class="sched__item">
        <span class="sched__item-dot" :class="{ 'sched__item-dot--on': w.enabled }" aria-hidden="true" />
        ...
        <button type="button" class="sched__remove" @click="remove(w.id)" aria-label="Remove window">×</button>
      </li>
    </ul>
...
interface Window { id: number; label: string; from: string; to: string; days: number[]; enabled: boolean }

const dayLabels = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']

let nextId = 3
const windows = reactive<Window[]>([
  { id: 1, label: 'Deep work block', from: '09:00', to: '12:00', days: [0, 1, 2, 3, 4], enabled: true },
  { id: 2, label: 'After hours', from: '18:00', to: '08:00', days: [0, 1, 2, 3, 4, 5, 6], enabled: true },
])
...
.sched__now-state--on { color: var(--accent); }
...
.sched__item-dot--on { background: var(--accent); }
...
.sched__item-toggle input { accent-color: var(--accent); }
...
.sched__day:hover { border-color: var(--accent); color: var(--accent); }
.sched__day--on {
...
.sched__remove:hover { color: var(--accent); border-color: var(--accent); }
...
.sched__add:hover { border-color: var(--accent); color: var(--accent); border-style: solid; }
</style>
