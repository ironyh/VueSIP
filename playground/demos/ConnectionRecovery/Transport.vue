<template>
  <div class="crt">
    <header class="crt__head">
      ...
      <div class="crt__ctrl">
        <button type="button" class="crt__btn" @click="drop" :disabled="state !== 'connected'">
          Drop link
        </button>
        <button type="button" class="crt__btn" @click="recover" :disabled="state === 'connected'">
          Force reconnect
        </button>
      </div>
    </header>
...
const now = () => {
  const d = new Date()
  return `${d.getMinutes().toString().padStart(2, '0')}:${d.getSeconds().toString().padStart(2, '0')}.${Math.floor(
