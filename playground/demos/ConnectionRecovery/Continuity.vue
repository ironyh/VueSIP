  ...
  <div class="crc">
    ...
    <section class="crc__banner" :class="`crc__banner--${phase.severity}`" role="status">
      ...
      <div class="crc__banner-actions">
        ...
        <button v-else type="button" class="crc__btn" @click="advance">Next step</button>
      </div>
    </section>
...
const phases: Record<PhaseId, Phase> = {
  ...
  reconnecting: {
    ...
    detail:
      'Transport dropped. Freeze transient UI, keep hangup visible, and reassure the user that recovery is in progress.',
    severity: 'warn',
    actions: [
  ...
  reregistering: {
    ...
    detail:
      'Media path is back, but the client still needs a fresh REGISTER before the app can trust new mid-call controls.',
    severity: 'warn',
    actions: [
  ...
  restored: {
    ...
    detail:
      'Recovery succeeded. Collapse the warning to a short-lived success state and return to the normal toolbar.',
    severity: 'ok',
    actions: [
  ...
  failed: {
    ...
    detail:
      'The automatic retries are exhausted. Surface a persistent failure state and ask the user to retry or redial.',
    severity: 'bad',
    actions: [
