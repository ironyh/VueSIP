      ...
      <ul class="crs__rows" role="list">
        <li class="crs__row">
          <label>
            ...
            <input type="number" min="5" max="300" v-model.number="state.wsPing" class="crs__input" />
            <span class="crs__u">seconds</span>
            <span class="crs__hint">CRLF frame on the SIP WebSocket; stock RFC 7118 behaviour.</span>
          </label>
        </li>
        <li class="crs__row">
          <label>
            <span class="crs__k">SIP OPTIONS</span>
            <input type="number" min="30" max="900" v-model.number="state.sipOptions" class="crs__input" />
            <span class="crs__u">seconds</span>
            <span class="crs__hint">Per-dialog heartbeat to the registrar. Also doubles as NAT binding refresh.</span>
          </label>
        </li>
      </ul>
    </section>
      ...
      <ul class="crs__rows" role="list">
        <li class="crs__row">
          <label>
            ...
            <input type="number" min="100" max="5000" v-model.number="state.backoffBase" class="crs__input" />
        ...
        <li class="crs__row">
          <label>
            ...
            <input type="number" min="1" max="20" v-model.number="state.maxAttempts" class="crs__input" />
        ...
        <li class="crs__row">
          <label>
            ...
            <span class="crs__hint">±50% on each attempt — prevents thundering herd after server restart.</span>
          </label>
        </li>
      </ul>
    </section>
...
const transports: Array<{ id: Fallback; label: string; desc: string }> = [
  ...
  { id: 'ws',   label: 'wss → ws',      desc: 'Drop TLS on retry. Last-resort only; auth might need rework.' },
  { id: 'tcp',  label: 'WSS → SIP/TCP', desc: 'Rare — requires a SIP-TCP edge your client can speak to directly.' },
...
const summary = computed(() => {
  const parts = [`ping ${state.wsPing}s`, `OPTIONS ${state.sipOptions}s`, `retry ×${state.maxAttempts}`]
...
.crs__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; flex-wrap: wrap; }
.crs__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.crs__title {
  margin: 0.1rem 0 0; font-size: 0.95rem; font-weight: 600;
  font-family: var(--mono); font-variant-numeric: tabular-nums; color: var(--accent);
  letter-spacing: 0.04em;
}

.crs__section { display: flex; flex-direction: column; gap: 0.4rem; }
.crs__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}

.crs__rows { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.35rem; }
.crs__row {
...
.crs__k { font-weight: 600; font-size: 0.88rem; }
.crs__input {
...
.crs__input:focus { outline: none; border-color: var(--accent); }
.crs__cb { accent-color: var(--accent); justify-self: start; }
.crs__u {
...
.crs__radio:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.crs__radio--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.crs__radio-label { font-weight: 600; font-size: 0.88rem; }
.crs__radio-desc {
