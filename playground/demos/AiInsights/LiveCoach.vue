  ...
  <div class="aic">
    ...
    <section class="aic__talk">
      <div class="aic__talk-bar" role="img" aria-label="Talk-listen ratio">
        <span class="aic__talk-agent" :style="{ width: `${talk.agent}%` }"
          >AGENT {{ talk.agent }}%</span
        >
        <span class="aic__talk-caller" :style="{ width: `${talk.caller}%` }"
          >CALLER {{ talk.caller }}%</span
        >
      </div>
      <span class="aic__talk-hint"
        >Target: caller ≥ 55%. Currently
        {{ talk.caller >= 55 ? 'healthy' : 'agent-dominated' }}.</span
      >
    </section>
  </div>
</template>
...
const nudges = ref([
  {
    ...
    kind: 'warn',
    t: 18,
    title: 'Acknowledge the frustration',
...
const sentTone = computed(() =>
  sentiment.value > 0.15 ? 'ok' : sentiment.value < -0.15 ? 'warn' : 'neutral'
)
const trendLabel = computed(() =>
  trend.value > 0.02 ? 'improving' : trend.value < -0.02 ? 'worsening' : 'flat'
)
...
.aic__meter-needle--warn {
  background: #b91c1c;
}
.aic__meter-needle--ok {
  background: #15803d;
}
.aic__meter-scale {
  display: flex;
...
.aic__nudge--warn .aic__nudge-kind {
  color: #b91c1c;
}
.aic__nudge--tip .aic__nudge-kind {
  color: var(--accent);
}
.aic__nudge-body strong {
  display: block;
  font-size: 0.84rem;
  margin-bottom: 0.1rem;
}
.aic__nudge-body small {
  display: block;
  font-family: var(--mono);
  font-size: 0.68rem;
  color: var(--muted);
