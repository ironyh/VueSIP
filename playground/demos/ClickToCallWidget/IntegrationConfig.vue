    ...
    <section class="wcfg__grid">
      <article class="wcfg__section">
        ...
        <label class="wcfg__field">
          ...
          <input v-model="config.callerName" class="wcfg__input" aria-label="Caller name" placeholder="Anonymous web visitor" />
        </label>
      </article>
      ...
      <article class="wcfg__section">
        ...
        <label class="wcfg__field">
          ...
          <div class="wcfg__themes" role="radiogroup" aria-label="Theme">
            ...
            >
              <span class="wcfg__theme-swatch" :style="{ background: t.swatch }" />
              {{ t.label }}
            </button>
          </div>
        </label>
      ...
      <article class="wcfg__section">
        ...
        <label class="wcfg__check">
          ...
          <span>
            ...
            <span class="wcfg__hint">WebRTC requires HTTPS; let the widget fail loudly if the host is misconfigured.</span>
          </span>
        </label>
        <label class="wcfg__check">
...
interface Theme { id: ThemeId; label: string; swatch: string }
...
.wcfg__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
...
.wcfg__copy:hover { background: var(--accent); }
.wcfg__copy--ok { background: var(--ok); }
...
.wcfg__field { display: flex; flex-direction: column; gap: 0.2rem; }
.wcfg__field > span {
  font-family: var(--mono);
  font-size: 0.6rem;
...
.wcfg__input:focus { outline: none; border-color: var(--accent); }
.wcfg__input--area {
  min-height: 3.5rem;
  line-height: 1.4;
  resize: vertical;
...
.wcfg__themes { display: inline-flex; flex-wrap: wrap; gap: 0.25rem; }
.wcfg__theme {
  display: inline-flex;
  align-items: center;
  gap: 0.35rem;
...
.wcfg__theme:hover { color: var(--ink); border-color: var(--ink); }
.wcfg__theme--on {
  color: var(--accent);
...
.wcfg__pills { display: inline-flex; gap: 0.2rem; }
.wcfg__pill {
  background: transparent;
  border: 1px solid var(--rule);
  border-radius: 2px;
...
.wcfg__pill:hover { color: var(--ink); border-color: var(--ink); }
.wcfg__pill--on {
  color: var(--accent);
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 8%, transparent);
...
.wcfg__check input { accent-color: var(--accent); margin-top: 0.25rem; }
.wcfg__hint {
  display: block;
  font-family: var(--mono);
  font-size: 0.64rem;
