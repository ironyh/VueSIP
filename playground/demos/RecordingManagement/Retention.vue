  ...
  <div class="ret">
    <header class="ret__head">
      <div>
        ...
        <h3 class="ret__title">
          {{ activeRules.length }} active rules · {{ holds.length }} on legal hold
        </h3>
      </div>
    </header>
        ...
        >
          ...
          <div class="ret__rule-tools">
            ...
            <label class="ret__switch">
              ...
              <span class="ret__switch-track" aria-hidden="true"
                ><span class="ret__switch-thumb"></span
              ></span>
    ...
    <section class="ret__section">
      ...
      <ul v-if="holds.length" class="ret__holds" role="list">
        <li v-for="h in holds" :key="h.id" class="ret__hold">
          ...
          <button type="button" class="ret__hold-release" @click="releaseHold(h.id)">
            Release
          </button>
        </li>
      </ul>
      <p v-else class="ret__empty-holds">
        No active legal holds. Add a hold when counsel issues a litigation-hold notice.
      </p>
      ...
      <ul v-if="exports.length" class="ret__exports" role="list">
        <li v-for="e in exports" :key="e.id" class="ret__export">
          <div class="ret__export-body">
            ...
            <span class="ret__export-meta"
              >{{ e.count }} recordings · {{ e.format.toUpperCase() }} · {{ e.destination }}</span
            >
          </div>
          <div class="ret__export-status">
            <div class="ret__progress">
              <div class="ret__progress-fill" :style="{ width: e.progress + '%' }"></div>
            </div>
            <span class="ret__export-state" :class="`ret__export-state--${e.state}`">{{
              e.state
            }}</span>
          </div>
        </li>
...
const rules = ref<Rule[]>([
  {
    id: 'default',
    scope: 'Default — all recordings',
    days: 90,
    desc: 'Applies if no more specific rule matches. Industry standard for general business.',
    enabled: true,
  },
  {
    id: 'support',
    scope: 'Support queue',
    days: 30,
    desc: 'Cheaper; support recordings are low legal risk once the ticket closes.',
    enabled: true,
  },
  {
    id: 'sales',
    scope: 'Sales queue',
    days: 365,
    desc: 'Kept a year for dispute resolution and contract confirmation.',
    enabled: true,
  },
  {
    id: 'vip',
    scope: 'VIP callers',
    days: 730,
    desc: 'Two years — escalation trail for enterprise accounts.',
    enabled: false,
  },
  {
    id: 'fin',
    scope: 'Financial / compliance calls',
    days: 2555,
    desc: '7 years — SEC Rule 17a-4, MiFID II equivalent.',
    enabled: false,
  },
])
...
const holds = ref<Hold[]>([
  {
    id: 1,
    caseId: 'LIT-2026-003',
    matter: 'Doe v. Example Corp — wrongful dismissal',
    count: 47,
    since: '2026-03-14',
  },
  {
    id: 2,
    caseId: 'REG-2026-011',
    matter: 'FCA enquiry — mis-selling review',
    count: 218,
    since: '2026-02-02',
  },
])
...
const exports = ref([
  {
    id: 'exp-1',
    label: 'Monthly QA batch — March',
    count: 40,
    format: 'mp3',
    destination: 's3://qa-export/2026-03',
    state: 'running',
    progress: 68,
  },
  {
    id: 'exp-2',
    label: 'LIT-2026-003 discovery',
    count: 47,
    format: 'wav',
    destination: 'sftp://counsel.example/intake',
    state: 'queued',
    progress: 0,
  },
  {
    id: 'exp-3',
    label: 'Training corpus v4',
    count: 1200,
    format: 'wav',
    destination: 'gs://ml-corpus/v4',
    state: 'done',
    progress: 100,
  },
] as {
...
.ret__switch input:checked + .ret__switch-track {
  background: var(--accent);
}
.ret__switch input:checked + .ret__switch-track .ret__switch-thumb {
...
.ret__export-state--running {
  color: var(--accent);
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 8%, transparent);
}
.ret__export-state--done {
  color: #047857;
  border-color: #047857;
  background: color-mix(in srgb, #047857 8%, transparent);
}
.ret__export-state--error {
  color: #b91c1c;
  border-color: #b91c1c;
