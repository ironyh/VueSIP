  ...
  <div class="sub">
    <header class="sub__head">
      <div>
        ...
        <h3 class="sub__title">
          {{ subscriptions.length }} active subscriptions · {{ trace.length }} events captured
        </h3>
      </div>
      <div class="sub__actions">
        ...
        >
          <span class="sub__btn-dot" aria-hidden="true"></span>
    ...
    <section class="sub__trace">
      ...
      <div class="sub__trace-body">
        <div
          v-for="(ev, i) in filteredTrace"
          :key="i"
          class="sub__event"
          :class="`sub__event--${ev.direction}`"
        >
          <div class="sub__event-head">
            <code class="sub__event-ts">{{ ev.ts }}</code>
            <span class="sub__event-dir">{{ ev.direction === 'out' ? '→' : '←' }}</span>
            <span
              class="sub__event-method"
              :class="`sub__event-method--${ev.method
                .toLowerCase()
                .replace(/\s+/g, '-')
                .replace(/[^a-z0-9-]/g, '')}`"
              >{{ ev.method }}</span
    ...
    <section class="sub__explain">
      ...
      <dl class="sub__explain-list">
        ...
        <dd>
          Client asks the registrar to notify it of mailbox changes. Refresh every
          <code>Expires</code> seconds or the subscription terminates.
        </dd>
        <dt><code>NOTIFY … application/simple-message-summary</code></dt>
        <dd>
          Server pushes the current mailbox state — message counts, urgent flag, account URI. Always
          idempotent, can be re-sent.
        </dd>
        <dt><code>Subscription-State: terminated;reason=noresource</code></dt>
        <dd>
          Mailbox deleted server-side. Client should stop re-SUBSCRIBEing and clear local state.
        </dd>
      </dl>
    </section>
  </div>
...
.sub__btn--recording .sub__btn-dot {
  background: var(--paper);
  animation: sub-blink 1s steps(2, start) infinite;
}
@keyframes sub-blink {
  50% {
    opacity: 0.2;
...
.sub__event-method--notify {
  background: var(--accent);
}
.sub__event-method--200-ok {
  background: #047857;
}
.sub__event-uri {
  font-family: var(--mono);
...
.sub__explain-list dt {
  font-family: var(--mono);
  font-size: 0.68rem;
  color: var(--accent);
  align-self: start;
}
.sub__explain-list dt code {
  font-family: inherit;
}
.sub__explain-list dd {
  margin: 0;
  font-size: 0.78rem;
  color: var(--ink);
  line-height: 1.45;
}
.sub__explain-list dd code {
  font-family: var(--mono);
  font-size: 0.85em;
  background: var(--paper);
  border: 1px solid var(--rule);
