  ...
  <div class="mwi">
    ...
    <div class="mwi__stage">
      <div class="mwi__lamp-wrap">
        <div
          class="mwi__lamp"
          :class="{ 'mwi__lamp--on': lampOn, 'mwi__lamp--urgent': urgentNew > 0 }"
          aria-hidden="true"
        >
          <span class="mwi__lamp-halo"></span>
          <span class="mwi__lamp-core"></span>
        </div>
    ...
    <section class="mwi__controls" aria-label="Simulate MWI change">
      ...
      <div class="mwi__control-grid">
        ...
        <button type="button" class="mwi__sim" @click="simulate(-1, 1, false)">
          Mark read (−1 new, +1 saved)
        </button>
        <button type="button" class="mwi__sim mwi__sim--primary" @click="clear">
          Clear all (lamp off)
        </button>
      </div>
    </section>
    ...
    <section class="mwi__accounts">
      ...
      <ul class="mwi__account-list" role="list">
        <li v-for="a in accounts" :key="a.uri" class="mwi__account">
          <div
            class="mwi__account-lamp"
            :class="{ 'mwi__account-lamp--on': a.new > 0 }"
            aria-hidden="true"
          ></div>
    ...
    <section class="mwi__rfc">
      ...
      <dl class="mwi__rfc-list">
        <dt>RFC 3842</dt>
        <dd>message-summary event package — carries the NOTIFY body</dd>
        <dt>RFC 3458</dt>
        <dd>Messages-Waiting / Voice-Message / Urgent-Messages headers</dd>
        <dt>RFC 3265</dt>
        <dd>SIP-specific event notification (SUBSCRIBE / NOTIFY mechanism)</dd>
      </dl>
    </section>
  </div>
</template>
...
.mwi__lamp--urgent .mwi__lamp-core {
  background: var(--urgent);
  box-shadow: 0 0 1.4rem 0.3rem color-mix(in srgb, var(--urgent) 45%, transparent);
}

@keyframes mwi-pulse {
  50% {
    box-shadow: 0 0 2rem 0.5rem color-mix(in srgb, var(--accent) 25%, transparent);
  }
}
...
.mwi__counter--new .mwi__counter-value {
  color: var(--accent);
}
.mwi__counter--urgent {
  border-left: 3px solid var(--urgent);
}
.mwi__counter--urgent .mwi__counter-value {
  color: var(--urgent);
}
