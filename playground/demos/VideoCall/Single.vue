<template>
  <div class="vsingle">
    <div class="vsingle__stage" :class="{ 'vsingle__stage--pip': layout === 'pip' }">
      <div class="vsingle__remote" :aria-label="`Remote video of ${remoteName}`">
        ...
        <span class="vsingle__quality" :class="`vsingle__quality--${quality}`">
          <span class="vsingle__q-dot" aria-hidden="true" />
      ...
      <div class="vsingle__self" :class="{ 'vsingle__self--off': selfOff }">
        ...
        <template v-else>
          <span class="vsingle__self-grid" aria-hidden="true">
            <span v-for="n in 6" :key="n" class="vsingle__self-bar" :style="{ height: `${20 + (n * 9) % 60}%` }" />
          </span>
    ...
    <dl class="vsingle__specs">
      <div><dt>Resolution</dt><dd>1280 × 720</dd></div>
      <div><dt>Bitrate</dt><dd>1.2 Mbps</dd></div>
      <div><dt>Codec</dt><dd>VP9</dd></div>
      <div><dt>RTT</dt><dd>48 ms</dd></div>
    </dl>
  </div>
</template>
...
const cycleCamera = () => { camIndex.value = (camIndex.value + 1) % cameras.length }
...
.vsingle__quality--fair .vsingle__q-dot { background: #eab308; }
.vsingle__quality--poor .vsingle__q-dot { background: var(--accent); }
...
.vsingle__stage--pip .vsingle__self { box-shadow: 0 8px 24px rgba(0,0,0,0.4); }
.vsingle__self-grid {
  display: inline-flex;
...
.vsingle__btn:hover:not(:disabled) { border-color: var(--accent); color: var(--accent); }
.vsingle__btn:disabled { opacity: 0.4; cursor: not-allowed; }
.vsingle__btn--off {
...
.vsingle__btn--end:hover:not(:disabled) { background: var(--ink); border-color: var(--ink); color: var(--paper); }
...
.vsingle__specs > div:last-child { border-right: 0; }
.vsingle__specs dt {
  font-family: var(--mono);
  font-size: 0.58rem;
  font-weight: 700;
...
@media (max-width: 620px) {
  .vsingle__specs { grid-template-columns: repeat(2, 1fr); }
  .vsingle__specs > div:nth-child(2) { border-right: 0; }
  .vsingle__specs > div:nth-child(3), .vsingle__specs > div:nth-child(4) { border-top: 1px solid var(--rule); }
}
</style>
