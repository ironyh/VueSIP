...
const initial: Caller[] = [
  ...
  {
    id: 5,
    number: 'sip:anon@unknown.invalid',
    joinedAt: Date.now() - 12000,
    waitSeconds: 12,
    position: 5,
    entry: 'direct',
  },
...
const sortedCallers = computed(() => [...callers.value].sort((a, b) => a.position - b.position))
...
const avgWait = computed(() => {
  ...
  return Math.round(callers.value.reduce((s, c) => s + c.waitSeconds, 0) / callers.value.length)
})
...
.qwait__row--crit {
  ...
  background: color-mix(in srgb, var(--crit) 4%, var(--paper));
}
