<template>
  <div class="alog">
      ...
      <ul class="alog__queue-list" role="list">
        ...
        >
          <div class="alog__queue-body">
            ...
            <span class="alog__queue-meta">
              penalty {{ q.penalty }} · {{ q.members }} agents
            </span>
          </div>
          <button
            type="button"
    ...
    <footer class="alog__footer">
      ...
      <button v-else type="button" class="alog__secondary" @click="logout">
        Log out
      </button>
      <span class="alog__hint">
        <span class="alog__dot" :class="{ 'alog__dot--on': loggedIn && !paused }" />
        {{ statusLine }}
      </span>
    </footer>
  </div>
...
interface StateOpt { id: StateId; label: string; code: string }
...
.alog__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.alog__timer {
  font-family: var(--mono);
  font-size: 1.05rem;
...
.alog__timer--paused { color: var(--muted); }
...
@media (max-width: 600px) { .alog__identity { grid-template-columns: 1fr; } }
.alog__field { display: flex; flex-direction: column; gap: 0.25rem; }
.alog__label {
  font-family: var(--mono);
  font-size: 0.62rem;
  font-weight: 700;
...
.alog__input:focus { outline: none; border-color: var(--accent); }
.alog__input:disabled { opacity: 0.7; cursor: not-allowed; }
...
.alog__queue-body { flex: 1; display: flex; flex-direction: column; gap: 0.15rem; }
.alog__queue-name { font-weight: 600; font-size: 0.9rem; }
.alog__queue-meta {
  font-family: var(--mono);
  font-size: 0.66rem;
...
.alog__queue-toggle:hover:not(:disabled) { color: var(--ink); border-color: var(--ink); }
.alog__queue-toggle:disabled { opacity: 0.5; cursor: not-allowed; }
...
@media (max-width: 600px) { .alog__seg { grid-template-columns: 1fr; } }
.alog__seg-btn {
  display: flex;
  flex-direction: column;
  gap: 0.15rem;
...
.alog__seg-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.alog__seg-label { font-weight: 600; font-size: 0.88rem; }
.alog__seg-code {
  font-family: var(--mono);
  font-size: 0.6rem;
  letter-spacing: 0.1em;
...
.alog__primary { background: var(--accent); color: var(--paper); }
.alog__primary:disabled { opacity: 0.4; cursor: not-allowed; }
.alog__secondary {
  background: transparent;
  color: var(--ink);
  border: 1px solid var(--ink);
}
.alog__secondary:hover { background: var(--ink); color: var(--paper); }
...
.alog__dot--on { background: #48bb78; }
</style>
