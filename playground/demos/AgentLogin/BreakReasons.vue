    ...
    <ul class="abrk__grid" role="list">
      <li v-for="r in reasons" :key="r.id">
        ...
        >
          ...
          <span v-if="r.cap" class="abrk__reason-cap">
            cap · {{ formatDuration(r.cap) }}
          </span>
        </button>
      </li>
    </ul>
...
const reasons: Reason[] = [
  ...
  { id: 'break', label: 'Short break', code: 'PAUSE · 02', desc: 'Ten-minute micro-break.', cap: 600 },
  { id: 'training', label: 'Training', code: 'PAUSE · 03', desc: 'Coaching or e-learning.', billable: true },
  { id: 'meeting', label: 'Meeting', code: 'PAUSE · 04', desc: 'Team huddle or 1:1.', billable: true },
  { id: 'bathroom', label: 'Bio', code: 'PAUSE · 05', desc: 'Not logged in detail.' },
  { id: 'acw', label: 'After-call work', code: 'PAUSE · 06', desc: 'Notes, CRM updates, dispositions.', billable: true },
]
...
const history = ref<HistoryEntry[]>([
  { id: 1, label: 'Short break', code: 'PAUSE · 02', duration: 420, endedAt: Date.now() - 2.4 * 3600_000 },
  { id: 2, label: 'Lunch', code: 'PAUSE · 01', duration: 1740, endedAt: Date.now() - 4 * 3600_000 },
  { id: 3, label: 'Training', code: 'PAUSE · 03', duration: 1800, endedAt: Date.now() - 6 * 3600_000 },
])
let nextId = 4
...
.abrk__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.abrk__timer {
  font-family: var(--mono);
  font-size: 1.1rem;
...
.abrk__timer--on { color: var(--accent); }
...
.abrk__reason:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.abrk__reason--on {
...
.abrk__reason-label { font-weight: 600; font-size: 0.88rem; }
.abrk__reason-code {
  font-family: var(--mono);
  font-size: 0.6rem;
...
.abrk__resume:disabled { opacity: 0.4; cursor: not-allowed; background: var(--rule); }

.abrk__totals {
  display: inline-flex;
  gap: 1.2rem;
}
.abrk__totals-item { display: flex; flex-direction: column; gap: 0.1rem; align-items: flex-end; }
.abrk__totals-label {
  font-family: var(--mono);
  font-size: 0.6rem;
  letter-spacing: 0.12em;
...
.abrk__history-label { font-size: 0.82rem; }
.abrk__history-dur {
  font-family: var(--mono);
  font-size: 0.78rem;
  font-weight: 600;
