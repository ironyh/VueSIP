<template>
  <div class="timer">
    ...
    <div v-if="!isActive" class="timer__idle" role="status">
      <span class="timer__idle-led" aria-hidden="true" />
      <span class="timer__idle-label">Timer</span>
      <span class="timer__idle-value">Idle</span>
      <span class="timer__idle-hint">
        {{ isSimulationMode ? 'Run the "Active Call" scenario' : 'Dial or answer a call to start the clock' }}
      </span>
    </div>

    <section v-else class="timer__stage" aria-live="polite" aria-atomic="true">
      <div class="timer__head">
        <span class="timer__pulse" aria-hidden="true" />
        <span class="timer__head-label">In call</span>
        <span class="timer__head-name">{{ effectiveRemoteDisplayName || effectiveRemoteUri || 'Unknown' }}</span>
      </div>
      ...
      <div class="timer__grid">
        ...
        <div class="timer__card">
          ...
          <span class="timer__card-value timer__card-value--prose">{{ formatHuman(effectiveDuration) }}</span>
...
.timer__idle-label { color: var(--muted); }
.timer__idle-value { color: var(--ink); font-weight: 700; }
.timer__idle-hint {
  margin-left: auto;
  color: var(--muted);
  text-transform: none;
