  ...
  <section class="breakdown" aria-live="polite">
    <div v-if="!isVisible" class="breakdown__idle" role="status">
      ...
      <span class="breakdown__idle-hint">
        {{ isSimulationMode ? 'Run the "Active Call" or "Hold" scenario' : 'Start a call to see active and hold time split out' }}
      </span>
    </div>
    ...
    <div v-else class="breakdown__stage">
      <div class="breakdown__head">
        ...
        <span class="breakdown__head-name">{{ effectiveRemoteDisplayName || effectiveRemoteUri || 'Unknown' }}</span>
      </div>
      ...
      <div class="breakdown__bar" aria-hidden="true">
        <div class="breakdown__bar-active" :style="{ width: activePct + '%' }" />
        <div class="breakdown__bar-hold" :style="{ width: holdPct + '%' }" />
      </div>
...
.breakdown__idle-label { color: var(--muted); }
.breakdown__idle-value { color: var(--ink); font-weight: 700; }
