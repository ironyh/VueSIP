  ...
  <div class="billable">
    ...
    <div v-if="!isActive" class="billable__idle">
      ...
      <span class="billable__idle-hint">
        {{ isSimulationMode ? 'Run the "Active Call" scenario to watch the meter tick' : 'Start a call to begin billing' }}
      </span>
    </div>
    ...
    <section v-else class="billable__stage" aria-live="polite" aria-atomic="true">
      <div class="billable__head">
        ...
        <span class="billable__head-name">{{ effectiveRemoteDisplayName || effectiveRemoteUri || 'Unknown' }}</span>
      </div>
      ...
      <div class="billable__hero">
        ...
        <span class="billable__hero-hint">{{ billableSeconds }}s billable · {{ effectiveDuration }}s elapsed</span>
      </div>
      ...
      <div class="billable__breakdown">
        ...
        <div>
          ...
          <span class="billable__breakdown-value">{{ effectiveDuration }}s → {{ billableSeconds }}s</span>
        </div>
      </div>
    </section>
  </div>
...
const billableUnits = computed(() =>
  Math.ceil(effectiveDuration.value / increment.value)
)
...
@media (max-width: 540px) {
  .billable__controls { grid-template-columns: auto 1fr; }
}
.billable__label {
...
.billable__idle-label { color: var(--muted); }
.billable__idle-value { color: var(--ink); font-weight: 700; }
.billable__idle-hint {
  margin-left: auto;
  color: var(--muted);
  text-transform: none;
...
@media (max-width: 540px) {
  .billable__breakdown { grid-template-columns: 1fr; }
}
