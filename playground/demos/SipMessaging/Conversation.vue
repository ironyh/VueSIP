  ...
  <div class="convo">
    ...
    <ol class="convo__stream" ref="streamEl" role="log" aria-live="polite">
      <li
        ...
        :key="m.id"
        class="convo__item"
        :class="{ 'convo__item--out': m.from === 'me' }"
      >
        <time v-if="showTimestamp(i)" class="convo__sep" :datetime="new Date(m.at).toISOString()">
          {{ dayLabel(m.at) }}
        </time>
      ...
      <li v-if="peerTyping" class="convo__item">
        <div class="convo__bubble convo__bubble--typing">
          <span class="convo__typing"> <span></span><span></span><span></span> </span>
        </div>
      </li>
    </ol>
    ...
    <form class="convo__composer" @submit.prevent="send">
      ...
      ></textarea>
      <button type="submit" class="convo__send" :disabled="!draft.trim()">Send</button>
    </form>
  </div>
</template>
...
const messages = ref<Message[]>([
  { id: 1, from: 'peer', text: 'Hey — are you in for the 3 PM review?', at: now - 3 * hour },
  {
    id: 2,
    from: 'me',
    text: 'Yes, should be on a call then. Dial me at this URI.',
    at: now - 3 * hour + 2 * min,
    status: 'read',
  },
  {
    id: 3,
    from: 'peer',
    text: 'Perfect. Bringing the mock-ups over.',
    at: now - 3 * hour + 5 * min,
  },
  {
    id: 4,
    from: 'me',
    text: "Great. I'll share my screen from the start.",
    at: now - 1 * hour,
    status: 'read',
  },
...
.convo__presence--online .convo__presence-dot {
  background: #48bb78;
}
.convo__presence--online {
  color: #48bb78;
}
.convo__presence--busy .convo__presence-dot {
  background: var(--accent);
}
.convo__presence--busy {
  color: var(--accent);
}
...
.convo__item--out .convo__receipt--read {
  color: color-mix(in srgb, var(--accent) 85%, var(--paper));
}

.convo__bubble--typing {
...
.convo__typing span:nth-child(2) {
  animation-delay: 0.15s;
}
.convo__typing span:nth-child(3) {
...
.convo__send:hover:not(:disabled) {
  background: var(--accent);
  border-color: var(--accent);
}
.convo__send:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
</style>
