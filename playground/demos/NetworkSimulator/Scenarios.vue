<template>
  <div class="ns">
    ...
    <footer class="ns__foot">
      <span>Applied via <code>chrome://inspect</code> throttling, a local proxy (e.g. <code>comcast</code>), or TURN-side impairment. The browser itself does not expose impairment APIs.</span>
    </footer>
  </div>
</template>
...
const scenarios: Scenario[] = [
  ...
  },
  {
    ...
    effect: 'Audio quality OK, but talk-over collisions increase dramatically — users will constantly interrupt each other.',
  },
  {
    id: 'lossy',
    label: 'Lossy uplink',
    ...
    effect: 'MOS 2.5 · Opus packet-loss concealment struggles; expect robotic audio and FEC bursts.',
  },
]
...
.ns__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; flex-wrap: wrap; }
.ns__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.ns__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.ns__stop {
...
.ns__stop:disabled { opacity: 0.4; cursor: not-allowed; }
.ns__stop:not(:disabled):hover { color: #b91c1c; border-color: #b91c1c; }
...
.ns__card:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.ns__card--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.ns__card-title { font-weight: 600; font-size: 0.95rem; }
.ns__card-desc {
...
.ns__kv dt { color: var(--muted); }
.ns__kv dd { margin: 0; color: var(--ink); font-variant-numeric: tabular-nums; }

.ns__effect { display: flex; flex-direction: column; gap: 0.3rem; }
.ns__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
