<template>
  <div class="nsc">
      ...
      <li v-for="s in sliders" :key="s.key" class="nsc__slider">
        <label>
          <span class="nsc__label">
            <span>{{ s.label }}</span>
            <span class="nsc__val"
              >{{ (state[s.key] as number).toFixed(s.decimals) }} {{ s.unit }}</span
            >
          </span>
          <input
            type="range"
    ...
    <footer class="nsc__foot">
      <span
        >Apply via <code>tc qdisc</code> (Linux), Network Link Conditioner (macOS), or Clumsy
        (Windows). Browser-side impairment is proxy-only.</span
      >
    </footer>
  </div>
</template>
...
const sliders: Slider[] = [
  {
    ...
    step: 5,
    decimals: 0,
    hint: 'Symmetric delay on the path. 150 ms is the ITU comfort ceiling.',
  },
  {
    key: 'jitter',
    label: 'Jitter',
    ...
    step: 1,
    decimals: 0,
    hint: 'Random variance on inter-arrival time. Ramps the jitter buffer.',
  },
  {
    key: 'loss',
    label: 'Packet loss',
    ...
    step: 0.1,
    decimals: 1,
    hint: 'Random drops. Opus PLC handles up to ~3%; past that audio degrades fast.',
  },
  {
    key: 'bandwidth',
    label: 'Bandwidth cap',
    ...
    step: 0.1,
    decimals: 1,
    hint: 'Hard ceiling on throughput. Triggers REMB/TWCC congestion control.',
  },
]
...
const effects = computed(() => {
  ...
  if (state.latency > 300) out.push('Talk-over collisions — users constantly interrupt each other')
    ...
    out.push('Noticeable but tolerable delay; natural conversation still works')
