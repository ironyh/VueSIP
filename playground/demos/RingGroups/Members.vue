  ...
  <div class="rgm">
    <header class="rgm__head">
      <div>
        ...
        <h3 class="rgm__title">
          {{ group.name }} · {{ reachable }} / {{ agents.length }} reachable
        </h3>
      </div>
    ...
    <ul class="rgm__agents" role="list">
      ...
      >
        <span class="rgm__dot" :class="`rgm__dot--${a.status}`" aria-hidden="true"></span>
    ...
    <footer class="rgm__foot">
      <span
        >In Asterisk terms: <code>Queue({{ group.ext }},,,,30)</code> or
        <code>Dial(PJSIP/a&amp;PJSIP/b&amp;PJSIP/c,30)</code> for ringall. Strategy maps to queue
        `strategy=` or to the ampersand list.</span
      >
    </footer>
  </div>
</template>
...
const strategies: Array<{ id: Strategy; label: string; desc: string }> = [
  {
    id: 'ringall',
    label: 'Ring all',
    desc: 'All phones ring simultaneously. Fastest pickup, loudest office.',
  },
  {
    id: 'hunt',
    label: 'Hunt',
    desc: 'Try in order; next on timeout. Classic receptionist escalation.',
  },
  {
    id: 'random',
    label: 'Random',
    desc: 'Shuffle on each call. Simple fairness without fairness math.',
  },
  {
    id: 'least-recent',
    label: 'Least-recent',
    desc: 'Oldest-idle agent first. Proper fairness for busy groups.',
  },
]
