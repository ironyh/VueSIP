<template>
  <div class="pgi">
      ...
      <ul class="pgi__targets" role="radiogroup" aria-label="Intercom target">
        <li v-for="t in targets" :key="t.ext">
          ...
          >
            ...
            <span
              class="pgi__target-status"
              :class="`pgi__target-status--${t.busy ? 'busy' : 'idle'}`"
            >
    ...
    <section class="pgi__controls">
      ...
      <button type="button" class="pgi__action" :disabled="state === 'idle'" @click="hangup">
        Hang up
      </button>
    ...
    <section class="pgi__timeline" aria-label="Connection timeline">
      <ol class="pgi__steps" role="list">
        <li
          ...
          :key="step.key"
          class="pgi__step"
          :class="{ 'pgi__step--on': reached(step.key) }"
        >
          <span class="pgi__step-dot" aria-hidden="true"></span>
    ...
    <footer class="pgi__foot">
      <span
        >Sends <code>Call-Info: answer-after=0</code>. Polycom and Yealink honour it; Cisco needs
        <code>Alert-Info: Intercom</code>. Sets <code>SIPAddHeader</code> in the dialplan before
        <code>Dial()</code>.</span
      >
    </footer>
  </div>
...
.pgi__target:hover:not(:disabled) {
