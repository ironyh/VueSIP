<template>
  <div class="pgm">
    ...
    <section class="pgm__ptt">
      ...
      >
        <span class="pgm__ptt-dot" />
        <span class="pgm__ptt-label">{{ state === 'live' ? 'Broadcasting…' : 'Hold to page' }}</span>
    ...
    <footer class="pgm__foot">
      <span>Asterisk <code>app_page()</code> with <code>MulticastRTP://239.1.2.3:5000</code>. Phones subscribe via <code>paging-group={{ selectedGroup.id }}</code> in their provisioning profile.</span>
    </footer>
  </div>
</template>
...
.pgm__head { display: flex; justify-content: space-between; align-items: center; gap: 0.75rem; flex-wrap: wrap; }
.pgm__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.pgm__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; letter-spacing: 0.04em; }
.pgm__state {
...
.pgm__section { display: flex; flex-direction: column; gap: 0.35rem; }
.pgm__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
...
.pgm__group:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.pgm__group--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.pgm__group-label { font-weight: 600; font-size: 0.9rem; }
.pgm__group-meta { font-family: var(--mono); font-size: 0.66rem; color: var(--muted); }

.pgm__ptt { display: flex; justify-content: center; padding: 0.5rem 0; }
.pgm__ptt-btn {
  user-select: none;
...
.pgm__ptt-btn:hover { border-color: var(--accent); }
.pgm__ptt-btn--on {
...
.pgm__ptt-btn--on .pgm__ptt-dot {
  background: var(--accent); animation: pgmPulse 1s ease-in-out infinite;
}
@keyframes pgmPulse { 50% { opacity: 0.35; } }
.pgm__ptt-label { font-size: 1rem; font-weight: 600; }
.pgm__ptt-hint { font-family: var(--mono); font-size: 0.64rem; color: var(--muted); letter-spacing: 0.08em; text-transform: uppercase; }

.pgm__log { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.25rem; }
.pgm__log-row {
  display: grid; grid-template-columns: 7rem 1fr 4rem 9rem;
  gap: 0.5rem; align-items: center;
  padding: 0.4rem 0.75rem;
  background: var(--paper); border: 1px solid var(--rule); border-radius: 2px;
}
@media (max-width: 600px) { .pgm__log-row { grid-template-columns: 1fr; } }
.pgm__log-at { font-family: var(--mono); font-size: 0.72rem; color: var(--muted); font-variant-numeric: tabular-nums; }
.pgm__log-group { font-size: 0.86rem; }
.pgm__log-dur {
  font-family: var(--mono); font-size: 0.7rem; color: var(--accent);
  font-variant-numeric: tabular-nums; text-align: right;
}
.pgm__log-by { font-family: var(--mono); font-size: 0.7rem; color: var(--muted); }
