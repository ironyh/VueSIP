  ...
  <div class="stats-demo">
    ...
    <ul class="stats-demo__kpis" role="list">
      <li class="stats-demo__kpi">
        ...
        <span class="stats-demo__kpi-sub">{{ totals.answered }} answered · {{ totals.missed }} missed</span>
      </li>
      <li class="stats-demo__kpi">
    ...
    <section class="stats-demo__block">
      ...
      <div class="stats-demo__histogram" role="img" :aria-label="`Call volume by hour, peak at ${peakHour}:00`">
        <div
    ...
    <section class="stats-demo__block">
      <header class="stats-demo__block-head">
        ...
        <span class="stats-demo__block-caption">top {{ topCallers.length }} of {{ uniqueCallers }}</span>
      </header>
        ...
        <li v-for="(row, i) in topCallers" :key="row.uri" class="stats-demo__ranking-row">
          ...
          <div class="stats-demo__ranking-body">
            <div class="stats-demo__ranking-head">
              ...
              <span class="stats-demo__ranking-count">{{ row.count }} {{ row.count === 1 ? 'call' : 'calls' }}</span>
            </div>
    ...
    <section class="stats-demo__block">
      ...
      <div class="stats-demo__split" role="img" :aria-label="`${totals.incoming} incoming, ${totals.outgoing} outgoing`">
        <span
          class="stats-demo__split-in"
          :style="{ flex: totals.incoming || 0.01 }"
        >
          <span class="stats-demo__split-label">In</span>
          <span class="stats-demo__split-value">{{ totals.incoming }}</span>
        </span>
        <span
          class="stats-demo__split-out"
          :style="{ flex: totals.outgoing || 0.01 }"
        >
...
.stats-demo__range-btn:hover { color: var(--ink); }
.stats-demo__range-btn--on { background: var(--ink); color: var(--paper); }
...
.stats-demo__bar-cell--peak .stats-demo__bar { background: var(--accent); }
...
.stats-demo__ranking { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.5rem; }
...
.stats-demo__ranking-name { font-size: 0.82rem; font-weight: 600; }
...
.stats-demo__split-label { opacity: 0.75; }
.stats-demo__split-value { font-weight: 700; font-variant-numeric: tabular-nums; }

@media (max-width: 560px) {
  .stats-demo__kpis { grid-template-columns: 1fr; }
}
</style>
