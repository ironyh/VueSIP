...
const entries = ref<LogEntry[]>([
  { id: 1, name: 'Alex Rivera', uri: 'sip:alex@example.com', direction: 'outgoing', kind: 'answered', at: now - 12 * min, duration: 248 },
  { id: 2, name: 'Unknown', uri: 'sip:+4670005551234@pstn.example.com', direction: 'incoming', kind: 'missed', at: now - 41 * min, duration: 0 },
  { id: 3, name: 'Front desk', uri: 'sip:100@pbx.example.com', direction: 'outgoing', kind: 'answered', at: now - 2 * hour, duration: 58 },
  { id: 4, name: 'Priya Shah', uri: 'sip:priya@example.com', direction: 'incoming', kind: 'answered', at: now - 3 * hour, duration: 612 },
  { id: 5, name: 'Sales team', uri: 'sip:200@pbx.example.com', direction: 'outgoing', kind: 'answered', at: now - 5 * hour, duration: 192 },
  { id: 6, name: 'Support on-call', uri: 'sip:911@pbx.example.com', direction: 'incoming', kind: 'missed', at: now - 1 * day, duration: 0 },
  { id: 7, name: 'Mei Chen', uri: 'sip:mei@example.com', direction: 'outgoing', kind: 'answered', at: now - 1 * day - 3 * hour, duration: 420 },
  { id: 8, name: 'Voicemail', uri: 'sip:*97@pbx.example.com', direction: 'outgoing', kind: 'answered', at: now - 2 * day, duration: 22 },
  { id: 9, name: 'Alex Rivera', uri: 'sip:alex@example.com', direction: 'incoming', kind: 'answered', at: now - 3 * day, duration: 95 },
  { id: 10, name: 'Unknown', uri: 'sip:+4670009998888@pstn.example.com', direction: 'incoming', kind: 'missed', at: now - 5 * day, duration: 0 },
])
...
const filters = computed<{ id: FilterId; label: string; count: number }[]>(() => [
  ...
  { id: 'incoming', label: 'Incoming', count: entries.value.filter((e) => e.direction === 'incoming' && e.kind !== 'missed').length },
  { id: 'outgoing', label: 'Outgoing', count: entries.value.filter((e) => e.direction === 'outgoing').length },
])
...
const filtered = computed(() => {
  ...
  return entries.value
    .filter((e) => {
      ...
      if (filter.value === 'incoming' && !(e.direction === 'incoming' && e.kind !== 'missed')) return false
...
const flash = (msg: string) => {
  ...
  statusTimer = setTimeout(() => { statusMessage.value = '' }, 2200)
}
...
.log-demo__filter:hover { color: var(--ink); }
.log-demo__filter--on { background: var(--ink); color: var(--paper); }
...
.log-demo__clear:hover { color: var(--accent); }

.log-demo__groups { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.6rem; }
...
.log-demo__entries { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.3rem; }
.log-demo__entry {
  display: grid;
...
.log-demo__entry:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
...
.log-demo__entry-body { min-width: 0; }
...
.log-demo__entry--missed .log-demo__entry-name { color: var(--accent); }
...
.log-demo__entry-dot { opacity: 0.55; }
.log-demo__entry-dir { text-transform: uppercase; letter-spacing: 0.1em; font-size: 0.6rem; }
.log-demo__entry-duration { font-variant-numeric: tabular-nums; }
...
.log-demo__tool:hover { color: var(--accent); border-color: var(--accent); }
...
.log-demo__empty-text { margin: 0; font-family: var(--mono); font-size: 0.75rem; }
...
@media (max-width: 560px) {
  .log-demo__entry { grid-template-columns: auto 1fr; row-gap: 0.3rem; }
  .log-demo__entry-tools { grid-column: 1 / 3; justify-content: flex-end; opacity: 1; }
}
</style>
