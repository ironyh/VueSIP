  ...
  <div class="cntd">
    <header class="cntd__head">
      <div class="cntd__avatar" :style="{ background: avatarColor(contact.name) }">
        ...
        <span class="cntd__presence" :class="`cntd__presence--${contact.presence}`"></span>
      </div>
      <div class="cntd__id">
        ...
        <div class="cntd__presence-row">
          <span
            class="cntd__presence-dot"
            :class="`cntd__presence--${contact.presence}`"
            aria-hidden="true"
          ></span>
          <span class="cntd__presence-label">{{ presenceLabel(contact.presence) }}</span>
          <span v-if="contact.presenceNote" class="cntd__presence-note"
            >— {{ contact.presenceNote }}</span
          >
        </div>
      </div>
    </header>
      ...
      <ul class="cntd__uris" role="list">
        <li v-for="u in contact.uris" :key="u.value" class="cntd__uri">
          ...
          <div class="cntd__uri-actions">
            <button
              type="button"
              class="cntd__btn cntd__btn--primary"
              :aria-label="`Call ${u.value}`"
            >
              Call
            </button>
            <button type="button" class="cntd__btn" :aria-label="`Copy ${u.value}`">Copy</button>
          </div>
        </li>
      </ul>
    </section>
