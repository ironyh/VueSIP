<template>
  <div class="cwa">
    ...
    <article class="cwa__panel cwa__panel--active">
      <div class="cwa__slot">
        ...
        <div class="cwa__slot-meta">
          <span
            >Codec <strong>{{ active.codec }}</strong></span
          >
    ...
    <section class="cwa__log">
      ...
      <ol>
        <li v-for="entry in log" :key="entry.id" class="cwa__log-row">
          <span class="cwa__log-dir" :class="`cwa__log-dir--${entry.dir}`">{{
    ...
    <p v-if="!waiting" class="cwa__hint">
      Second line idle. Click <strong>Ring L2</strong> to simulate an incoming INVITE arriving
...
onMounted(() => {
  tick = window.setInterval(() => activeDuration.value++, 1000)
})
onUnmounted(() => {
...
const toggleHold = () => {
  ...
  push(
    'tx',
    activeHeld.value ? 're-INVITE · a=sendonly · Line 1' : 're-INVITE · a=sendrecv · Line 1'
  )
}
...
.cwa__slot--waiting .cwa__slot-label {
  color: var(--accent);
}
.cwa__slot-body {
  display: flex;
...
.cwa__op--primary .cwa__op-glyph {
  color: var(--accent);
}
.cwa__op--danger:hover:not(:disabled) {
  border-color: var(--danger);
}
.cwa__op--danger .cwa__op-glyph {
  color: var(--danger);
}
