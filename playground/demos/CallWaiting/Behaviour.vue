  ...
  <div class="cwb">
    <header class="cwb__head">
      ...
      <label class="cwb__master">
        ...
        <span class="cwb__master-track" aria-hidden="true">
          <span class="cwb__master-thumb"></span>
        </span>
    ...
    <section class="cwb__section">
      ...
      <div class="cwb__counter">
        ...
        <span class="cwb__counter-hint"
          >Including the active one. Set to 1 to behave like a pre-2005 desk phone.</span
        >
      </div>
    </section>
    ...
    <section class="cwb__section">
      ...
      <ul class="cwb__opts" role="list">
        ...
        <li class="cwb__opt">
          ...
          <span class="cwb__opt-hint"
            >Any keypress or click kills the tone even without answering.</span
          >
        </li>
        <li class="cwb__opt">
          <label class="cwb__opt-label">
...
const secondBehaviour: { id: SecondId; label: string; desc: string; code: string }[] = [
  {
    id: 'tone-only',
    label: 'Tone, wait for user',
    desc: 'Play local call-waiting tone. User picks up or rejects manually.',
    code: 'SIP 180 Ringing',
  },
  {
    id: 'tone-auto-hold',
    label: 'Tone, auto-hold on answer',
    desc: 'When user answers, hold the active call automatically.',
    code: 're-INVITE a=sendonly',
  },
  {
    id: 'auto-answer',
    label: 'Auto-hold, auto-answer',
    desc: 'Aggressive — useful for dispatcher intercom.',
    code: 'session.accept()',
  },
  {
    id: 'reject',
    label: 'Silently reject',
    desc: 'Never ring a second call.',
    code: 'SIP 486 Busy Here',
  },
]
...
const overflowBehaviour: { id: OverflowId; label: string; desc: string; code: string }[] = [
  {
    id: 'busy',
    label: 'Busy Here',
    desc: 'Return 486; caller hears busy or goes to their own voicemail.',
    code: 'SIP 486',
  },
  {
    id: 'voicemail',
    label: 'Forward to voicemail',
    desc: 'Server-side redirect to vm@domain.',
    code: 'SIP 302 Moved · Contact: sip:vm@…',
  },
  {
    id: 'forward',
    label: 'Forward to coverage number',
    desc: 'Redirect to a colleague or mobile.',
    code: 'SIP 302 · Contact: sip:+…',
  },
  {
    id: 'silent',
    label: 'Silent drop',
    desc: 'Accept then immediately BYE — hides your presence.',
    code: '200 OK → BYE',
  },
]
...
.cwb__master input:checked + .cwb__master-track {
  background: var(--accent);
}
.cwb__master input:checked + .cwb__master-track .cwb__master-thumb {
...
.cwb__counter-btn:hover:not(:disabled) {
  color: var(--accent);
  border-color: var(--accent);
}
.cwb__counter-btn:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
.cwb__counter-v {
...
.cwb__summary-text strong {
  color: var(--ink);
  font-weight: 700;
}
.cwb__summary-text em {
  color: var(--accent);
  font-style: normal;
  font-weight: 600;
}
