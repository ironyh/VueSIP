<template>
  <div class="spr">
    ...
    <section class="spr__controls">
      ...
      <span class="spr__hint"
        >Uses <code>sessionStorage</code> for the call dialog snapshot;
        <code>localStorage</code> for preferences.</span
      >
    </section>
  </div>
</template>
...
const steps = [
  {
    key: 'dehydrate',
    label: 'Dehydrate',
    desc: 'Call dialog snapshot written to sessionStorage on `pagehide`.',
    time: '−0.0s',
  },
  {
    key: 'reload',
    label: 'Tab reload',
    desc: 'JS heap lost; SIP peer connection closed.',
    time: '0.0s',
  },
  {
    key: 'rehydrate',
    label: 'Rehydrate',
    desc: 'App boots, reads snapshot, restores Pinia store.',
    time: '0.3s',
  },
  {
    key: 'connect',
    label: 'WebSocket reconnect',
    desc: 'Transport up; no registrar interaction yet.',
    time: '0.8s',
  },
  {
    key: 'register',
    label: 'REGISTER refresh',
    desc: 'Fresh CSeq; auth via stored credentials; contact reuses instance-id.',
    time: '1.1s',
  },
  {
    key: 'rehail',
    label: 'Dialog re-hail',
    desc: 'UPDATE or re-INVITE with existing `Call-ID`; remote stays on the line.',
    time: '1.6s',
  },
  {
    key: 'live',
    label: 'Media re-established',
    desc: 'ICE restart finishes; RTP flows; UI restores controls.',
    time: '2.2s',
  },
]
...
.spr__step--done .spr__step-dot {
  background: #15803d;
  border-color: #15803d;
}
.spr__step--on .spr__step-dot {
