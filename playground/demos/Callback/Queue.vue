  ...
  <div class="cbq">
    ...
    <ul v-if="pending.length" class="cbq__list" role="list">
      <li
        ...
        :key="cb.id"
        class="cbq__row"
        :class="[`cbq__row--${cb.priority}`, { 'cbq__row--overdue': isOverdue(cb), 'cbq__row--calling': cb.status === 'calling' }]"
      >
        <div class="cbq__row-rank">
          <span class="cbq__pri" :class="`cbq__pri--${cb.priority}`">{{ priorityLabel(cb.priority) }}</span>
        </div>
        <div class="cbq__row-main">
          ...
          <div class="cbq__row-meta">
            <span>Queue <strong>{{ cb.queue }}</strong></span>
            <span class="cbq__sep" aria-hidden="true">·</span>
            <span>ETA <strong>{{ formatEta(cb.scheduledAt) }}</strong></span>
            <span class="cbq__sep" aria-hidden="true">·</span>
...
const requests = ref<Callback[]>([
  { id: 1, number: '+14155550142', name: 'Rachel Kim', queue: 'sales', priority: 'vip', scheduledAt: Date.now() - 4 * MIN, attempts: 1, maxAttempts: 5, status: 'pending', lastResult: 'no-answer' },
  { id: 2, number: '+442079460031', name: 'David Owens', queue: 'support', priority: 'high', scheduledAt: Date.now() + 3 * MIN, attempts: 0, maxAttempts: 3, status: 'pending' },
  { id: 3, number: '+13105550998', queue: 'support', priority: 'normal', scheduledAt: Date.now() + 12 * MIN, attempts: 0, maxAttempts: 3, status: 'pending' },
  { id: 4, number: '+442079461122', name: 'Anita Patel', queue: 'billing', priority: 'high', scheduledAt: Date.now() + 22 * MIN, attempts: 2, maxAttempts: 3, status: 'pending', lastResult: 'busy' },
  { id: 5, number: '+4915205557001', queue: 'sales', priority: 'normal', scheduledAt: Date.now() + 45 * MIN, attempts: 0, maxAttempts: 3, status: 'pending' },
])
...
const addSample = () => {
  const samples = [
    { number: '+16175551234', name: 'Jordan Alvarez', queue: 'sales', priority: 'normal' as Priority },
    { number: '+33145550066', name: 'Céline Dufour', queue: 'billing', priority: 'high' as Priority },
...
.cbq__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; font-variant-numeric: tabular-nums; }
.cbq__overdue { color: var(--danger); font-weight: 700; }
.cbq__new {
  background: var(--ink); color: var(--paper); border: 0; border-radius: 2px;
  padding: 0.5rem 0.85rem; font-family: var(--mono); font-size: 0.68rem;
  letter-spacing: 0.1em; text-transform: uppercase; cursor: pointer;
}
.cbq__new:hover { background: var(--accent); }
...
.cbq__list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.35rem; }
.cbq__row {
  display: grid;
...
.cbq__row--vip { border-left-color: var(--vip); background: color-mix(in srgb, var(--vip) 3%, var(--paper)); }
.cbq__row--high { border-left-color: var(--accent); }
.cbq__row--overdue { border-color: var(--danger); }
.cbq__row--calling { border-color: var(--accent); background: color-mix(in srgb, var(--accent) 8%, var(--paper)); }

.cbq__row-rank { display: flex; }
.cbq__pri {
  font-family: var(--mono); font-size: 0.58rem; font-weight: 700;
  letter-spacing: 0.12em; padding: 0.18rem 0.4rem; border-radius: 2px;
  background: var(--paper-deep); border: 1px solid var(--rule); color: var(--muted);
}
.cbq__pri--vip { background: var(--vip); color: var(--paper); border-color: var(--vip); }
.cbq__pri--high { background: color-mix(in srgb, var(--accent) 18%, transparent); border-color: var(--accent); color: var(--accent); }

.cbq__row-main { display: flex; flex-direction: column; gap: 0.2rem; min-width: 0; }
.cbq__row-head { display: flex; align-items: baseline; gap: 0.5rem; flex-wrap: wrap; }
.cbq__num { font-family: var(--mono); font-size: 0.88rem; font-weight: 600; font-variant-numeric: tabular-nums; }
.cbq__name { font-size: 0.85rem; color: var(--ink); }
.cbq__row-meta {
  display: inline-flex; flex-wrap: wrap; align-items: center; gap: 0.35rem;
  font-family: var(--mono); font-size: 0.66rem; color: var(--muted);
}
.cbq__row-meta strong { color: var(--ink); font-weight: 600; }
.cbq__sep { opacity: 0.5; }
.cbq__last { color: var(--danger); text-transform: uppercase; letter-spacing: 0.08em; }

.cbq__row-tools { display: inline-flex; gap: 0.3rem; flex-shrink: 0; }
.cbq__tool {
  background: transparent; border: 1px solid var(--rule); border-radius: 2px;
  padding: 0.4rem 0.6rem; font-family: var(--mono); font-size: 0.62rem;
  letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted);
  cursor: pointer; transition: all 0.12s;
}
.cbq__tool:hover { color: var(--ink); border-color: var(--ink); }
.cbq__tool--primary { color: var(--accent); border-color: var(--accent); background: color-mix(in srgb, var(--accent) 6%, transparent); }
.cbq__tool--primary:hover { background: var(--accent); color: var(--paper); }
.cbq__tool--primary:disabled { opacity: 0.55; cursor: wait; }
.cbq__tool--danger { font-size: 1rem; line-height: 1; padding: 0.2rem 0.55rem; }
.cbq__tool--danger:hover { color: var(--danger); border-color: var(--danger); }
