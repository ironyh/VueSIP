<template>
  <div class="cbp">
    ...
    <section class="cbp__section">
      ...
      <div class="cbp__retry">
        <label class="cbp__field">
          ...
          <input type="number" v-model.number="retry.max" min="1" max="10" class="cbp__input cbp__input--num" />
        </label>
        <label class="cbp__field">
          <span class="cbp__label">Between tries</span>
          <div class="cbp__stepper">
            <input type="number" v-model.number="retry.intervalMin" min="1" class="cbp__input cbp__input--num" />
    ...
    <section class="cbp__section">
      ...
      <ul class="cbp__guards" role="list">
        <li class="cbp__guard">
          ...
          <span class="cbp__guard-hint">Cap: {{ guards.rateLimitPerHour }}/hr across all agents.</span>
        </li>
        <li class="cbp__guard">
          <label class="cbp__guard-label">
    ...
    <footer class="cbp__foot">
      <span class="cbp__foot-dot" :class="{ 'cbp__foot-dot--on': openNow }" />
      <span class="cbp__foot-label">
        <strong>{{ openNow ? 'Inside business hours' : 'Outside business hours' }}</strong> ·
        final failure → <em>{{ currentFailLabel }}</em> · {{ retry.max }} attempts at {{ retry.intervalMin }}m {{ retry.backoff }}
      </span>
    </footer>
  </div>
</template>
...
const retry = reactive({ max: 3, intervalMin: 15, backoff: 'linear' as 'fixed' | 'linear' | 'expo' })
...
const failActions: { id: FailId; label: string; desc: string; code: string }[] = [
  { id: 'voicemail', label: 'To voicemail', desc: 'Drop a pre-recorded message and close the request.', code: 'AMI Originate → vm context' },
  { id: 'notify', label: 'Notify agent', desc: 'Push to the assigned agent; they decide whether to retry.', code: 'WebSocket · X-Callback-Retry' },
  { id: 'drop', label: 'Mark abandoned', desc: 'No further action; close with status=failed.', code: 'status=failed · reason=max-retries' },
  { id: 'email', label: 'Email summary', desc: 'Send the caller a rescheduling link.', code: 'SMTP · template cb-missed' },
]
...
.cbp__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; }
.cbp__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.cbp__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }

.cbp__section { display: flex; flex-direction: column; gap: 0.45rem; }
.cbp__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
...
.cbp__field { display: flex; flex-direction: column; gap: 0.25rem; }
.cbp__label {
...
.cbp__input {
  ...
  padding: 0.44rem 0.6rem; font-family: var(--mono); font-size: 0.85rem; color: var(--ink);
  font-variant-numeric: tabular-nums;
}
.cbp__input:focus { outline: none; border-color: var(--accent); }
.cbp__input--num { max-width: 100%; }
.cbp__input--select { padding: 0.38rem 0.5rem; }
.cbp__stepper { display: inline-flex; align-items: center; gap: 0.4rem; }
.cbp__unit {
  font-family: var(--mono); font-size: 0.68rem;
  color: var(--muted); letter-spacing: 0.08em; text-transform: uppercase;
}

.cbp__days { display: flex; gap: 0.3rem; flex-wrap: wrap; }
.cbp__day {
  padding: 0.35rem 0.65rem; background: var(--paper); border: 1px solid var(--rule);
  border-radius: 2px; font-family: var(--mono); font-size: 0.65rem;
  letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted);
  cursor: pointer; transition: all 0.12s;
}
.cbp__day:hover { color: var(--ink); border-color: var(--ink); }
.cbp__day--on {
...
.cbp__preview {
  ...
  padding: 0.55rem 0.7rem; display: flex; flex-direction: column; gap: 0.35rem;
}
.cbp__preview-label {
  font-family: var(--mono); font-size: 0.6rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}
.cbp__preview-row { display: flex; flex-wrap: wrap; gap: 0.3rem; }
.cbp__chip {
  display: inline-flex; align-items: center; gap: 0.35rem;
  padding: 0.18rem 0.45rem; background: var(--paper);
  border: 1px solid var(--rule); border-radius: 2px;
  font-family: var(--mono); font-size: 0.7rem;
  font-variant-numeric: tabular-nums;
}
.cbp__chip-n { color: var(--muted); }
.cbp__chip-t { color: var(--accent); font-weight: 700; }
...
.cbp__action:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.cbp__action--on {
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.cbp__action-label { font-weight: 600; font-size: 0.88rem; }
.cbp__action-desc { font-family: var(--mono); font-size: 0.66rem; color: var(--muted); }
.cbp__action-code {
  font-family: var(--mono); font-size: 0.6rem;
  letter-spacing: 0.08em; color: var(--accent); margin-top: 0.15rem;
}

.cbp__guards { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.35rem; }
.cbp__guard {
  background: var(--paper); border: 1px solid var(--rule); border-radius: 2px;
...
.cbp__guard-label input { accent-color: var(--accent); }
.cbp__guard-hint {
...
.cbp__foot {
  ...
  padding: 0.55rem 0.75rem; background: var(--paper-deep);
  border: 1px solid var(--rule); border-radius: 2px;
}
.cbp__foot-dot {
  width: 8px; height: 8px; border-radius: 50%; background: var(--muted);
}
.cbp__foot-dot--on { background: var(--ok); }
.cbp__foot-label {
  font-family: var(--mono); font-size: 0.7rem; color: var(--muted); letter-spacing: 0.03em;
}
.cbp__foot-label strong { color: var(--ink); font-weight: 700; }
.cbp__foot-label em { color: var(--accent); font-style: normal; }
</style>
