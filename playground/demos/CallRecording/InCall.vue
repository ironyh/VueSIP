  ...
  <div class="rec">
    <header class="rec__head">
      <div class="rec__head-main">
        ...
        <span class="rec__callid">Call-ID {{ session.callId }} · {{ formatDuration(callElapsed) }}</span>
      </div>
      <div class="rec__indicator" :class="{ 'rec__indicator--on': state === 'recording' }" aria-hidden="true">
        <span class="rec__dot" />
        <span class="rec__indicator-label">{{ state === 'recording' ? 'REC' : state === 'paused' ? 'PSE' : 'OFF' }}</span>
      </div>
    </header>

    <div class="rec__stage">
      <div class="rec__meter" :class="`rec__meter--${state}`" aria-hidden="true">
        <span v-for="(b, i) in bars" :key="i" class="rec__bar" :style="{ height: b + '%' }" />
      </div>
    ...
    <section class="rec__legal">
      <label class="rec__legal-row">
        ...
        <div>
          ...
          <span class="rec__legal-hint">Plays "{{ announcement }}" when recording starts. Required in two-party consent jurisdictions.</span>
        </div>
      </label>
      <label class="rec__legal-row">
        ...
        <div>
          ...
          <span class="rec__legal-hint">1400 Hz, 200 ms tone injected into local mix. Cheapest defensible notice.</span>
        </div>
      </label>
      <label class="rec__legal-row">
        ...
        <div>
          ...
          <span class="rec__legal-hint">Set after verbal acknowledgment. Disables the record button until true.</span>
        </div>
      </label>
    </section>
...
const stateLabel = computed(() =>
  state.value === 'recording' ? 'Recording' : state.value === 'paused' ? 'Paused' : state.value === 'stopped' ? 'Stopped' : 'Idle'
)
...
.rec__head-main { display: flex; flex-direction: column; gap: 0.15rem; min-width: 0; }
.rec__eyebrow {
...
.rec__title { margin: 0.1rem 0 0; display: inline-flex; gap: 0.6rem; align-items: center; flex-wrap: wrap; font-size: 1rem; font-weight: 600; }
.rec__party {
  font-family: var(--mono);
  font-size: 0.9rem;
...
.rec__state--recording { color: var(--accent); border-color: var(--accent); background: color-mix(in srgb, var(--accent) 10%, transparent); }
.rec__state--paused { color: #8b7500; border-color: #8b7500; background: color-mix(in srgb, #8b7500 8%, transparent); }
...
@media (prefers-reduced-motion: reduce) {
  .rec__indicator--on .rec__dot { animation: none; }
}
...
.rec__indicator--on .rec__indicator-label { color: var(--accent); }
...
.rec__bar {
  ...
  background: var(--muted);
  border-radius: 1px;
  transition: height 0.18s ease-out, background 0.15s;
  min-height: 2px;
}
.rec__meter--recording .rec__bar { background: var(--accent); }
.rec__meter--paused .rec__bar { background: color-mix(in srgb, var(--accent) 40%, var(--muted)); }

.rec__timer { text-align: right; }
.rec__timer-label {
  display: block;
  font-family: var(--mono);
  font-size: 0.6rem;
...
.rec__controls { display: flex; flex-wrap: wrap; gap: 0.4rem; }
.rec__btn {
  display: inline-flex;
  align-items: center;
  gap: 0.45rem;
...
.rec__btn:hover:not(:disabled) { border-color: var(--ink); }
.rec__btn:disabled { opacity: 0.5; cursor: not-allowed; }
.rec__btn--record { background: var(--accent); color: var(--paper); border-color: var(--accent); }
.rec__btn--record:hover:not(:disabled) { background: var(--ink); border-color: var(--ink); }
.rec__btn--stop { background: var(--ink); color: var(--paper); border-color: var(--ink); }
.rec__btn-icon { font-size: 0.85rem; }
...
.rec__legal-row input { margin-top: 0.2rem; accent-color: var(--accent); }
.rec__legal-title { display: block; font-weight: 600; color: var(--ink); }
.rec__legal-hint {
  display: block;
  margin-top: 0.15rem;
  font-family: var(--mono);
...
.rec__log-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.15rem; }
.rec__log-row { display: grid; grid-template-columns: auto auto 1fr; gap: 0.6rem; align-items: baseline; font-family: var(--mono); font-size: 0.7rem; }
.rec__log-ts { color: color-mix(in srgb, var(--paper) 45%, transparent); font-variant-numeric: tabular-nums; }
.rec__log-kind {
  font-size: 0.58rem;
  font-weight: 700;
  text-transform: uppercase;
...
.rec__log-kind--info { background: color-mix(in srgb, var(--paper) 12%, transparent); color: color-mix(in srgb, var(--paper) 80%, transparent); }
.rec__log-kind--send { background: color-mix(in srgb, var(--accent) 80%, transparent); color: var(--paper); }
.rec__log-kind--warn { background: #b45309; color: var(--paper); }
.rec__log-msg { color: var(--paper); }
</style>
