  ...
  <div class="pol">
    <header class="pol__head">
      <div>
        ...
        <h3 class="pol__title">
          {{ enabledRules }} of {{ rules.length }} auto-record rules active
        </h3>
      </div>
      ...
      <ul class="pol__rules" role="list">
        ...
        >
          ...
          <label class="pol__switch">
            ...
            <span class="pol__switch-track" aria-hidden="true"
              ><span class="pol__switch-thumb"></span
            ></span>
...
const rules = ref<Rule[]>([
  {
    id: 'inbound',
    name: 'All inbound calls',
    match: 'direction=in',
    desc: 'Records every answered inbound, regardless of destination or queue.',
    enabled: true,
  },
  {
    id: 'outbound',
    ...
    match: 'direction=out',
    desc: 'Records every outbound. Often disabled for regulatory reasons in the EU.',
    enabled: false,
  },
  {
    id: 'queue-sales',
    name: 'Sales queue',
    match: 'queue in ("sales","vip-sales")',
    desc: 'Recorded for coaching and dispute resolution.',
    enabled: true,
  },
  {
    id: 'queue-support',
    name: 'Support queue',
    match: 'queue="support"',
    desc: 'Records support calls for QA sampling.',
    enabled: true,
  },
  {
    id: 'vip',
    name: 'VIP callers',
    match: 'from matches /^\\+1415555(01\\d\\d)$/',
    desc: 'Escalation recording for known VIPs (Priya Shah, etc.).',
    enabled: false,
  },
  {
    id: 'afterhours',
    name: 'After-hours',
    match: 'time not in business_hours',
    desc: 'Records overflow to on-call engineer.',
    enabled: true,
  },
])
...
const consentModes = [
  {
    ...
    label: 'Announcement',
    desc: 'IVR plays consent text at call start',
  },
...
const policyPreview = computed(() => {
  ...
  return [
    ...
    consentMode.value === 'announcement'
      ? `  Playback(consent-notice)  # "${consentText.value.slice(0, 40)}…"`
      : '',
...
.pol__switch input:checked + .pol__switch-track {
  background: var(--accent);
}
.pol__switch input:checked + .pol__switch-track .pol__switch-thumb {
