<template>
  <div class="cbs">
    ...
    <section class="cbs__section">
      ...
      <div class="cbs__row">
        ...
        <label class="cbs__field">
          ...
          <input
            type="text"
            v-model="bridge.recordPath"
            class="cbs__input"
            :disabled="!bridge.record"
          />
        </label>
      </div>
    </section>
    ...
    <footer class="cbs__foot">
      <span
        >Effective on next <code>ConfBridge()</code> entry — existing members keep their current
        profile.</span
      >
    </footer>
  </div>
...
const toggles: Array<{ key: keyof Bridge; label: string; hint: string }> = [
  ...
  {
    key: 'endMarked',
    label: 'End when marked leaves',
    hint: 'Bridge closes when all admins disconnect.',
  },
