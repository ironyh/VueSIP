  ...
  <div class="grid-demo">
    <div class="grid-demo__head">
      ...
      <div class="grid-demo__tools">
        ...
        <button type="button" class="grid-demo__reset" @click="clearAll" :disabled="filledCount === 0">
          Clear all
        </button>
      </div>
    </div>
    ...
    <ul class="grid-demo__grid" role="list">
      <li v-for="(slot, idx) in speedDialSlots" :key="idx" class="grid-demo__cell">
        ...
        <div v-else class="grid-demo__slot" :class="{ 'grid-demo__slot--calling': callingIndex === idx }">
          <button
            type="button"
    ...
    <div v-if="editingIndex !== null" class="grid-demo__sheet" role="dialog" aria-label="Edit speed dial slot">
      <div class="grid-demo__sheet-head">
        <span class="grid-demo__sheet-eyebrow">
          Slot {{ editingIndex + 1 }} · {{ editingContact ? 'Edit' : 'New' }}
        </span>
        <button type="button" class="grid-demo__sheet-close" @click="closeDialog" aria-label="Close">×</button>
      </div>
      ...
      <div class="grid-demo__sheet-actions">
        ...
        <button type="button" class="grid-demo__sheet-secondary" @click="closeDialog">Cancel</button>
      </div>
    </div>
  </div>
</template>
...
interface SpeedDialContact { name: string; number: string }
...
const flashStatus = (msg: string) => {
  ...
  statusTimer = setTimeout(() => { statusMessage.value = '' }, 2400)
}
...
const dial = (contact: SpeedDialContact, idx: number) => {
  ...
  setTimeout(() => { callingIndex.value = null }, 1200)
}
...
.grid-demo__reset:hover:not(:disabled) { color: var(--accent); border-color: var(--accent); }
.grid-demo__reset:disabled { opacity: 0.45; cursor: not-allowed; }
...
.grid-demo__slot:hover { border-color: color-mix(in srgb, var(--accent) 50%, var(--rule)); }
...
@keyframes grid-demo-pulse {
  0%, 100% { box-shadow: 0 0 0 2px color-mix(in srgb, var(--accent) 16%, transparent); }
...
.grid-demo__dial:hover:not(:disabled) { background: var(--paper-deep); }
.grid-demo__dial:disabled { cursor: not-allowed; opacity: 0.55; }
...
.grid-demo__slot-tool:hover { color: var(--accent); border-color: var(--accent); }
...
.grid-demo__sheet-close:hover { color: var(--accent); }

.grid-demo__field { display: flex; flex-direction: column; gap: 0.25rem; }
...
.grid-demo__sheet-primary:hover:not(:disabled) { background: var(--accent); border-color: var(--accent); }
.grid-demo__sheet-primary:disabled { opacity: 0.45; cursor: not-allowed; }
.grid-demo__sheet-secondary {
  background: transparent;
  color: var(--muted);
  border: 1px solid var(--rule);
}
.grid-demo__sheet-secondary:hover { color: var(--accent); border-color: var(--accent); }

@media (max-width: 560px) {
  .grid-demo__grid { grid-template-columns: repeat(2, 1fr); }
}
</style>
