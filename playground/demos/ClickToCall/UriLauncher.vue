<template>
  <div class="c2c">
    ...
    <div class="c2c__sample">
      ...
      <div class="c2c__sample-body" v-html="highlighted" />
    </div>
    ...
    <section class="c2c__section">
      ...
      <ul v-if="links.length" class="c2c__list" role="list">
        ...
        >
          ...
          <button type="button" class="c2c__call" :aria-label="`Call ${link.display}`" @click="launch(link)">
            Call
          </button>
        </li>
      </ul>
    ...
    <footer v-if="lastLaunched" class="c2c__receipt" role="status" aria-live="polite">
      ...
      <span class="c2c__receipt-note">Browser handler chain: <code>navigator.registerProtocolHandler</code> → OS default app</span>
    </footer>
  </div>
</template>
...
const sampleText =
  `Alex Rivera · Senior Account Executive · Acme Industrial
...
.c2c__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }

.c2c__scheme-pick { display: inline-flex; gap: 0.25rem; }
.c2c__scheme {
...
.c2c__scheme:hover { color: var(--ink); border-color: var(--ink); }
.c2c__scheme--on {
  color: var(--accent);
...
.c2c__section { display: flex; flex-direction: column; gap: 0.4rem; }
.c2c__section-title {
  font-family: var(--mono);
  font-size: 0.64rem;
  font-weight: 700;
...
.c2c__row--tel { border-left-color: var(--tel); }
.c2c__row--sip { border-left-color: var(--sip); }
.c2c__row--callto { border-left-color: var(--callto); }
...
.c2c__receipt-note { color: var(--muted); font-size: 0.64rem; letter-spacing: 0.05em; }
</style>
