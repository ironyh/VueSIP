    ...
    <header class="c2cp__head">
      <div>
        ...
        <h3 class="c2cp__title">
          {{ parsed.length }} found · {{ selectedCount }} selected · {{ ambiguousCount }} need
          review
        </h3>
      </div>
    ...
    <section class="c2cp__section">
      ...
      <ul v-if="parsed.length" class="c2cp__list" role="list">
        <li
          ...
          :key="i"
          class="c2cp__row"
          :class="{ 'c2cp__row--amb': p.ambiguous }"
        >
          <input
            type="checkbox"
            :checked="p.selected"
      ...
      <p v-else class="c2cp__empty">
        Paste text above. Parser recognises E.164, NANP, EU national, and common extensions.
      </p>
    </section>

    <footer class="c2cp__dial">
      <button type="button" class="c2cp__dial-btn" :disabled="selectedCount === 0" @click="dial">
