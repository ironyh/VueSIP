  ...
  <div class="pkf">
    ...
    <dl class="pkf__grid">
      <div class="pkf__field">
        ...
        <dd>
          <input v-model="cfg.parkExt" type="text" inputmode="numeric" class="pkf__input pkf__input--short" />
      ...
      <div class="pkf__field">
        ...
        <dd>
          <input v-model="cfg.timeout" type="number" min="10" max="600" class="pkf__input pkf__input--short" />
          <span class="pkf__hint">After this, the call returns to the parker. 45s is the classic default.</span>
        </dd>
      </div>
      <div class="pkf__field">
        <dt>Return rule</dt>
      ...
      <div class="pkf__field">
        ...
        <dd>
          ...
          <span class="pkf__hint">Parkees hear this; choose something neutral. Rickrolls are a firing offence.</span>
        </dd>
      </div>
      <div class="pkf__field">
        ...
        <dd>
          <label class="pkf__toggle">
            ...
            <span>{{ cfg.announce ? 'Speak slot number back' : 'Silent park (display only)' }}</span>
          </label>
          <span class="pkf__hint">Silent park works only if the handset shows slot info on its screen — half of them don't.</span>
        </dd>
      </div>
    </dl>
    ...
    <section class="pkf__preview">
      ...
      <pre class="pkf__code">[parkinglot_{{ cfg.name }}]
context = parkedcalls
parkext = {{ cfg.parkExt }}
...
.pkf__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.5rem; flex-wrap: wrap; }
.pkf__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.16em; text-transform: uppercase; color: var(--muted);
}
.pkf__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; letter-spacing: 0.04em; }
...
.pkf__field dd { margin: 0; display: flex; flex-direction: column; gap: 0.25rem; }
...
.pkf__input--short { max-width: 6rem; }
.pkf__row { display: flex; align-items: center; gap: 0.4rem; font-family: var(--mono); font-size: 0.78rem; color: var(--muted); }
.pkf__hint { font-family: var(--mono); font-size: 0.66rem; color: var(--muted); }

.pkf__options { display: grid; gap: 0.3rem; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); }
.pkf__option {
...
.pkf__option:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.pkf__option--on {
  border-color: var(--accent); background: color-mix(in srgb, var(--accent) 6%, transparent);
}
.pkf__option-label { font-weight: 600; font-size: 0.8rem; }
.pkf__option-desc { font-family: var(--mono); font-size: 0.62rem; color: var(--muted); }
