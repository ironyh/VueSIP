<template>
  <div class="umd">
    <header class="umd__head">
      <div>
        <span class="umd__eyebrow"
          >Directory · {{ users.length }} users · {{ onlineCount }} online</span
        >
    ...
    <footer class="umd__foot">
      <span
        >Auth source: <code>pjsip_auths</code> realtime table · endpoints:
        <code>pjsip_endpoints</code>. Mix of SSO (SAML/OIDC) and local PJSIP secrets; plain
        passwords are for service extensions only.</span
      >
    </footer>
  </div>
</template>
...
const onlineCount = computed(
  () => users.filter((u) => u.status === 'online' || u.status === 'busy').length
)
...
const statusLabel = (s: Status) =>
  s === 'online' ? 'online' : s === 'busy' ? 'on call' : s === 'away' ? 'away' : 'offline'
</script>
...
.umd__status--online .umd__dot {
  background: #15803d;
}
.umd__status--busy .umd__dot {
  background: var(--accent);
}
.umd__status--away .umd__dot {
  background: #ca8a04;
}
.umd__status--offline .umd__dot {
  background: #6b5d4a;
  opacity: 0.4;
}
