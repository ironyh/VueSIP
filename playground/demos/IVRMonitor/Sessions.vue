  ...
  <div class="ivs">
    ...
    <ul class="ivs__list" role="list">
      <li
        ...
        :class="[
          `ivs__row--${s.status}`,
          { 'ivs__row--ab': s.status === 'abandoned' },
        ]"
      >
...
interface Step { node: string; dtmf?: string }
interface Session {
  id: string
  callId: string
  number: string
...
const sampleFlows: Array<{ path: Step[]; status: Status; outcome?: string }> = [
  { path: [{ node: 'root', dtmf: '1' }, { node: 'main', dtmf: '2' }, { node: 'support', dtmf: '2' }, { node: 'queue' }], status: 'transferred', outcome: '→ support queue' },
  { path: [{ node: 'root', dtmf: '1' }, { node: 'main', dtmf: '3' }, { node: 'billing', dtmf: '1' }], status: 'completed', outcome: 'self-serve balance' },
  { path: [{ node: 'root', dtmf: '1' }, { node: 'main', dtmf: '2' }, { node: 'support' }], status: 'abandoned', outcome: 'timeout at support menu' },
  { path: [{ node: 'root', dtmf: '1' }, { node: 'main', dtmf: '0' }, { node: 'queue' }], status: 'transferred', outcome: '→ operator' },
...
.ivs__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }

.ivs__controls { display: inline-flex; gap: 0.3rem; }
.ivs__ctrl {
  background: var(--ink);
  color: var(--paper);
  border: 0;
...
.ivs__ctrl:hover { background: var(--accent); }
.ivs__ctrl--ghost {
  background: transparent;
  color: var(--muted);
  border: 1px solid var(--rule);
}
.ivs__ctrl--ghost:hover { color: var(--ink); border-color: var(--ink); background: transparent; }
...
.ivs__row--navigating { border-left-color: var(--accent); }
.ivs__row--transferred { border-left-color: var(--ok); }
.ivs__row--completed { border-left-color: color-mix(in srgb, var(--ok) 60%, var(--rule)); }
.ivs__row--abandoned { border-left-color: var(--crit); opacity: 0.85; }
...
.ivs__status--navigating { color: var(--accent); background: color-mix(in srgb, var(--accent) 10%, transparent); }
.ivs__status--transferred { color: var(--ok); background: color-mix(in srgb, var(--ok) 10%, transparent); }
.ivs__status--completed { color: color-mix(in srgb, var(--ok) 70%, var(--ink)); background: color-mix(in srgb, var(--ok) 8%, transparent); }
.ivs__status--abandoned { color: var(--crit); background: color-mix(in srgb, var(--crit) 10%, transparent); }
...
.ivs__step { display: inline-flex; align-items: center; gap: 0.25rem; }
.ivs__step-node {
  color: var(--ink);
...
.ivs__step-arrow { color: var(--muted); }
.ivs__cursor {
  color: var(--accent);
...
.ivs__sep { opacity: 0.5; }
.ivs__outcome { color: var(--ink); font-weight: 600; }
