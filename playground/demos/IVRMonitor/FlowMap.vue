    ...
    <header class="ivf__head">
      ...
      <span class="ivf__legend-wrap">
        <span class="ivf__legend">
          <span class="ivf__legend-dot ivf__legend-dot--hot" /> high traffic
        </span>
        <span class="ivf__legend">
          <span class="ivf__legend-dot ivf__legend-dot--drop" /> drop-off &gt; 15%
        </span>
      </span>
    </header>
      ...
      <li v-for="node in nodes" :key="node.id" class="ivf__level">
        ...
        >
          ...
          <dl class="ivf__node-stats">
            <div><dt>Visits</dt><dd>{{ node.visits.toLocaleString() }}</dd></div>
            <div><dt>Avg time</dt><dd>{{ node.avgTime }}s</dd></div>
            <div><dt>Drop-off</dt>
              <dd :class="{ 'ivf__stat--bad': node.dropRate > 15 }">
                {{ node.dropRate }}%
              </dd>
            </div>
          </dl>
          <div v-if="node.options" class="ivf__opts">
            ...
            >
              ...
              <span class="ivf__opt-bar" aria-hidden="true">
                <span class="ivf__opt-fill" :style="{ width: opt.pct + '%' }" />
              </span>
...
interface Option { digit: string; label: string; pct: number }
interface Node {
  id: string
  name: string
  prompt: string
...
const nodes: Node[] = [
  ...
  },
  {
    id: 'support',
    name: 'Support triage',
    prompt: 'For account issues press 1. For technical support press 2. To hear more options, press 9.',
    visits: 788,
    avgTime: 18,
    dropRate: 22,
    options: [
  ...
  },
  {
    id: 'billing',
    name: 'Billing menu',
    prompt: 'For your balance press 1, to make a payment press 2, for all other billing questions press 0.',
    visits: 302,
    avgTime: 9,
    dropRate: 6,
    options: [
...
.ivf__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.ivf__legend-wrap { display: inline-flex; gap: 0.8rem; flex-wrap: wrap; }
.ivf__legend {
  display: inline-flex;
  align-items: center;
  gap: 0.35rem;
...
.ivf__legend-dot { width: 10px; height: 10px; border-radius: 2px; }
.ivf__legend-dot--hot { background: var(--accent); }
.ivf__legend-dot--drop { background: var(--crit); }
...
.ivf__node:focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; }
.ivf__node--hot { border-left-color: var(--accent); }
.ivf__node--drop { border-left-color: var(--crit); background: color-mix(in srgb, var(--crit) 3%, var(--paper)); }
.ivf__node--selected { outline: 2px solid var(--accent); outline-offset: -1px; }
...
.ivf__node-name { font-weight: 600; font-size: 0.92rem; }
...
.ivf__node-stats div { display: flex; flex-direction: column; gap: 0.1rem; margin: 0; }
.ivf__node-stats dt {
  font-family: var(--mono);
  font-size: 0.58rem;
  letter-spacing: 0.1em;
...
.ivf__stat--bad { color: var(--crit); }

.ivf__opts { display: flex; flex-direction: column; gap: 0.25rem; }
.ivf__opt {
  display: grid;
...
.ivf__opt-label { font-size: 0.78rem; }
.ivf__opt-bar {
...
.ivf__detail-text strong { color: var(--accent); font-weight: 700; }
</style>
