<template>
  <div class="fcc">
    ...
    <ul class="fcc__list" role="list">
      <li v-for="(c, i) in codes" :key="i" class="fcc__row" :class="{ 'fcc__row--conflict': c.conflict }">
        <input
          v-model="c.code"
          type="text"
        ...
        <input
          ...
          aria-label="Target"
        />
        <button type="button" class="fcc__remove" @click="codes.splice(i, 1)" aria-label="Remove">×</button>
...
exten =&gt; {{ c.code || '...' }},1,NoOp(custom: {{ c.label || '(unnamed)' }})
 ...
 same =&gt; n,Hangup()</template>
    ...
    <footer class="fcc__foot">
      <span>Built-ins win every conflict. Reserve <code>*0*</code> and <code>*9*</code> for your tenant; use two-digit codes only for true global verbs.</span>
    </footer>
  </div>
</template>
...
const builtIns = ['##', '*2', '700', '*5', '*97', '*98', '*72', '*73', '*90', '*91', '*67', '*69', '*78', '*79', '*80']

interface Custom { code: string; label: string; action: string; target: string; conflict: string | null }

const codes = reactive<Custom[]>([
  { code: '*411', label: 'Directory lookup',           action: 'goto',  target: 'directory-app,s,1',          conflict: null },
  { code: '*800', label: 'Emergency broadcast',        action: 'macro', target: 'macro-emergency-page',       conflict: null },
  { code: '*901', label: 'Toggle night mode',          action: 'set',   target: 'NIGHTMODE/main=1',           conflict: null },
  { code: '*5',   label: 'Custom one-touch',           action: 'dial',  target: 'PJSIP/supervisor',           conflict: null },
])
...
const renderAction = (c: Custom) => {
  switch (c.action) {
    case 'dial':  return `Dial(${c.target || '...'})`
    case 'goto':  return `Goto(${c.target || '...'})`
    case 'macro': return `Macro(${c.target || '...'})`
    case 'set':   return `Set(${c.target || '...'})`
    default: return ''
  }
}
...
.fcc__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.5rem; flex-wrap: wrap; }
.fcc__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.16em; text-transform: uppercase; color: var(--muted);
}
.fcc__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; font-variant-numeric: tabular-nums; }
.fcc__add {
...
.fcc__add:hover { background: var(--accent); color: var(--paper); border-color: var(--accent); }

.fcc__list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.35rem; }
.fcc__row {
  display: grid;
...
@media (max-width: 700px) { .fcc__row { grid-template-columns: 1fr; } }
.fcc__row--conflict { border-color: #b91c1c; background: color-mix(in srgb, #b91c1c 5%, transparent); }
...
.fcc__input--code { font-weight: 700; color: var(--accent); text-align: center; }
.fcc__input--label { font-family: var(--sans); }
.fcc__input--target { font-size: 0.72rem; }
...
.fcc__remove:hover { color: #b91c1c; border-color: #b91c1c; }

.fcc__warn {
  grid-column: 1 / -1;
  font-family: var(--mono); font-size: 0.68rem; color: #b91c1c;
  padding-top: 0.2rem;
}

.fcc__preview { display: flex; flex-direction: column; gap: 0.35rem; }
.fcc__preview-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
