<template>
  <div class="aar">
    <header class="aar__head">
      ...
      <label class="aar__master">
        ...
        <span class="aar__master-track" aria-hidden="true">
          <span class="aar__master-thumb"></span>
        </span>
    ...
    <p class="aar__hint">
      Rules evaluate top-to-bottom on <code>INVITE</code>. First match wins. Asterisk sends intercom
      calls with <code>Call-Info: &lt;sip:…&gt;;answer-after=0</code> — the classic trigger.
    </p>
    ...
    <ul class="aar__list" role="list">
      <li
        ...
        :key="r.id"
        class="aar__row"
        :class="{ 'aar__row--off': !r.enabled || !masterEnabled }"
      >
        ...
        <label class="aar__switch">
          ...
          <span class="aar__switch-track" aria-hidden="true">
            <span class="aar__switch-thumb"></span>
          </span>
    ...
    <section class="aar__sim">
      ...
      <div
        v-if="simResult"
        class="aar__sim-out"
        :class="{ 'aar__sim-out--hit': simResult.matched }"
      >
...
const runMatch = () => {
  if (!masterEnabled.value) {
    simResult.value = {
      matched: false,
      message: 'Master toggle is off — call would ring normally.',
    }
    return
  }
...
.aar__master input:checked + .aar__master-track {
  background: var(--accent);
}
.aar__master input:checked + .aar__master-track .aar__master-thumb {
...
.aar__switch input:checked + .aar__switch-track {
  background: var(--accent);
}
.aar__switch input:checked + .aar__switch-track .aar__switch-thumb {
