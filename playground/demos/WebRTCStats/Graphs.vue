<template>
  <div class="wsg">
      ...
      <article v-for="s in series" :key="s.id" class="wsg__card">
        ...
        <svg class="wsg__chart" viewBox="0 0 120 36" preserveAspectRatio="none" aria-hidden="true">
          <polyline
            ...
            fill="none"
            :stroke="tier(s) === 'ok' ? 'var(--accent)' : tier(s) === 'warn' ? '#d97706' : '#b91c1c'"
            stroke-width="1.4"
    ...
    <footer class="wsg__foot">
      <span>All metrics extracted from <code>remote-inbound-rtp</code> and <code>candidate-pair</code>. Store raw samples for at least 5 min if you want p95 to stabilise.</span>
    </footer>
  </div>
</template>
...
const initSeries = (): Series[] => [
  { id: 'rtt',     label: 'RTT',           unit: 'ms',   decimals: 0, min: 0,  max: 400, warn: 150, bad: 300, values: noisy(80, 25, 60) },
  { id: 'jitter',  label: 'Jitter (peer)', unit: 'ms',   decimals: 1, min: 0,  max: 100, warn: 30,  bad: 60,  values: noisy(14, 8,  60) },
  { id: 'loss',    label: 'Packet loss',   unit: '%',    decimals: 2, min: 0,  max: 10,  warn: 1,   bad: 3,   values: noisy(0.3, 0.4, 60).map((v) => Math.max(0, v)) },
  { id: 'bitrate', label: 'Bitrate (out)', unit: 'kbps', decimals: 0, min: 0,  max: 80,  warn: 1e9, bad: 1e9, values: noisy(48, 6, 60) },
  { id: 'fps',     label: 'Frames / s',    unit: 'fps',  decimals: 0, min: 0,  max: 40,  warn: 20,  bad: 10,  values: noisy(28, 3, 60) },
  { id: 'nack',    label: 'NACK count',    unit: '/s',   decimals: 0, min: 0,  max: 20,  warn: 5,   bad: 10,  values: noisy(1, 2, 60).map((v) => Math.max(0, v)) },
]
...
.wsg__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; flex-wrap: wrap; }
.wsg__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.wsg__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.wsg__sample {
...
.wsg__card-head { display: flex; justify-content: space-between; align-items: baseline; }
.wsg__k {
...
.wsg__v--warn { color: #d97706; }
.wsg__v--bad { color: #b91c1c; }
.wsg__u { font-family: var(--mono); font-size: 0.65rem; color: var(--muted); font-weight: 400; margin-left: 0.15rem; }
