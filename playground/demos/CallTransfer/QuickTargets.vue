  ...
  <section class="quick-transfer" aria-live="polite">
    <div v-if="!isActive" class="quick-transfer__gate">
      ...
      <span class="quick-transfer__gate-hint">
        {{
          isSimulationMode
            ? 'Run the "Active Call" scenario in the first variant'
            : 'Connect a call to enable quick-transfer buttons'
        }}
      </span>
    </div>
    ...
    <div v-else class="quick-transfer__stage">
      <div class="quick-transfer__head">
        ...
        <span class="quick-transfer__head-name">{{
      ...
      <p
        v-if="status"
        class="quick-transfer__status"
        :class="{ 'quick-transfer__status--error': isError }"
      >
        {{ status }}
      </p>
    </div>
...
.quick-transfer__gate-label {
  color: var(--muted);
}
.quick-transfer__gate-value {
  font-weight: 700;
}
