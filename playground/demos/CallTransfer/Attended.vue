  ...
  <div class="attended">
    ...
    <section v-else class="attended__stage" aria-live="polite">
      ...
      <div v-else class="attended__actions">
        ...
        <button v-if="step < 3" type="button" class="attended__btn" @click="cancel">
...
const consultName = computed(() => {
  ...
  return (
    controls.consultationCall.value?.remoteDisplayName ||
...
const beginConsult = async () => {
  ...
  try {
    ...
    step.value = 1
    await controls.initiateAttendedTransfer(callSession.session.value.id, target.value.trim())
    step.value = 2
  } catch (err) {
    step.value = 0
...
.attended__gate-label {
  color: var(--muted);
}
.attended__gate-value {
  color: var(--ink);
  font-weight: 700;
}
.attended__gate-hint {
