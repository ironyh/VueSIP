      ...
      <div class="picker__group">
        ...
        <ul v-else class="picker__list" role="radiogroup" aria-label="Microphone">
          <li v-for="d in inputs" :key="d.deviceId">
            ...
            >
              <span class="picker__item-dot" aria-hidden="true" />
              <span class="picker__item-name">{{ d.label || `Microphone ${d.deviceId.slice(0, 6)}` }}</span>
      ...
      <div class="picker__group">
        ...
        <ul v-else class="picker__list" role="radiogroup" aria-label="Speaker">
          <li v-for="d in outputs" :key="d.deviceId">
            ...
            >
              <span class="picker__item-dot" aria-hidden="true" />
              <span class="picker__item-name">{{ d.label || `Speaker ${d.deviceId.slice(0, 6)}` }}</span>
              <span class="picker__item-id">{{ d.deviceId.slice(0, 10) }}</span>
            </button>
          </li>
        </ul>
...
const inputs = computed(() =>
  isSimulationMode.value ? mockIn.value : audioInputDevices.value
)
const outputs = computed(() =>
  isSimulationMode.value ? mockOut.value : audioOutputDevices.value
)
...
const pickInput = (id: string) => {
  if (isSimulationMode.value) { mockSelIn.value = id; flash('Microphone switched'); return }
  selectAudioInput(id); flash('Microphone switched')
}
const pickOutput = (id: string) => {
  if (isSimulationMode.value) { mockSelOut.value = id; flash('Speaker switched'); return }
  selectAudioOutput(id); flash('Speaker switched')
}

const refresh = async () => {
  if (isSimulationMode.value) { flash('Device list refreshed'); return }
  try { await enumerateDevices(); flash('Device list refreshed') }
  catch { flash('Refresh failed') }
}

onMounted(async () => {
  if (isSimulationMode.value) { granted.value = true; return }
  try {
    await enumerateDevices()
...
.picker__gate-value { color: var(--ink); font-weight: 600; }
.picker__gate-btn {
  margin-left: auto;
...
.picker__gate-btn:hover { background: var(--accent); border-color: var(--accent); }
...
.picker__head-refresh:hover { border-color: var(--accent); color: var(--accent); }

.picker__group { display: flex; flex-direction: column; gap: 0.4rem; }
.picker__group-label {
  font-family: var(--mono);
  font-size: 0.65rem;
  font-weight: 700;
