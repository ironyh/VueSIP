...
const scenarios = [
  {
    ...
    detail: 'The UI must stop before the user hits “Call” and explain that `getUserMedia` is unavailable on plain HTTP.',
  ...
  },
  {
    ...
    detail: 'Once HTTPS is in place, the remaining failures should come from browser permission state or missing devices.',
...
const scenario = computed(() => scenarios.find((option) => option.id === current.value) ?? scenarios[0])
