    ...
    <section class="pfo__shell" :class="`pfo__shell--${state.severity}`">
      ...
      <ul class="pfo__devices">
        <li
          ...
          class="pfo__device"
          :class="{
            'pfo__device--offline': !device.available,
            'pfo__device--selected': device.selected,
          }"
        >
...
const scenarios = [
  {
    ...
    detail:
      'The browser cannot expose a separate output list yet. Fall back gracefully and explain the limit.',
  ...
  },
  {
    ...
    detail:
      'The app cannot promise playback. It should surface the problem before the user places a call.',
