<template>
  <div class="astat">
    ...
    <section class="astat__split">
      <span class="astat__section-title"
        >Time split · logged-in {{ formatDuration(shiftSeconds) }}</span
      >
      <div
        class="astat__bar"
        role="img"
        :aria-label="`Talk ${pct(talk)}%, hold ${pct(hold)}%, wrap ${pct(wrap)}%, idle ${pct(idle)}%`"
      >
        <span class="astat__seg astat__seg--talk" :style="{ width: pct(talk) + '%' }"></span>
        <span class="astat__seg astat__seg--hold" :style="{ width: pct(hold) + '%' }"></span>
        <span class="astat__seg astat__seg--wrap" :style="{ width: pct(wrap) + '%' }"></span>
        <span class="astat__seg astat__seg--idle" :style="{ width: pct(idle) + '%' }"></span>
      </div>
      <ul class="astat__legend" role="list">
        <li><span class="astat__dot astat__dot--talk"></span>Talk · {{ formatDuration(talk) }}</li>
        <li><span class="astat__dot astat__dot--hold"></span>Hold · {{ formatDuration(hold) }}</li>
        <li><span class="astat__dot astat__dot--wrap"></span>Wrap · {{ formatDuration(wrap) }}</li>
        <li><span class="astat__dot astat__dot--idle"></span>Idle · {{ formatDuration(idle) }}</li>
      </ul>
    </section>
    ...
    <section class="astat__adherence">
      ...
      <div class="astat__bar astat__bar--thin">
        <span class="astat__seg astat__seg--adh" :style="{ width: adherence + '%' }"></span>
      </div>
      <div class="astat__adh-row">
        <span class="astat__adh-value">{{ adherence }}%</span>
        <span
          class="astat__adh-hint"
          :class="
            adherence >= 90
              ? 'astat__adh-hint--good'
              : adherence >= 80
                ? 'astat__adh-hint--warn'
                : 'astat__adh-hint--bad'
          "
        >
          {{ adherenceLabel }}
...
.astat__seg--wrap {
  background: color-mix(in srgb, var(--accent) 30%, var(--paper-deep));
}
.astat__seg--idle {
  background: var(--rule);
}
