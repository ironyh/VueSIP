  ...
  <div class="cdra">
    <header class="cdra__head">
      <div>
        ...
        <h3 class="cdra__title">{{ totalCalls }} calls · {{ totalMin }} min · ${{ totalCost.toFixed(2) }}</h3>
      </div>
    </header>
    ...
    <section class="cdra__section">
      ...
      <div class="cdra__bars" role="img" aria-label="Daily call volume">
        <div v-for="d in volume" :key="d.day" class="cdra__day">
          <div class="cdra__stack">
            <div class="cdra__inb"  :style="{ height: (d.inb / max) * 100 + '%' }" :title="`${d.inb} inbound`" />
            <div class="cdra__out" :style="{ height: (d.out / max) * 100 + '%' }" :title="`${d.out} outbound`" />
          </div>
      ...
      <div class="cdra__legend">
        <span><i class="cdra__sw cdra__sw--inb" /> Inbound</span>
        <span><i class="cdra__sw cdra__sw--out" /> Outbound</span>
      </div>
    </section>
      ...
      <ul class="cdra__dests" role="list">
        <li v-for="d in destinations" :key="d.prefix" class="cdra__dest">
          ...
          <span class="cdra__meter">
            <span class="cdra__meter-fill" :style="{ width: (d.calls / destinations[0].calls) * 100 + '%' }" />
          </span>
...
interface Day { day: string; inb: number; out: number }
...
.cdra__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; flex-wrap: wrap; }
.cdra__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.cdra__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; font-variant-numeric: tabular-nums; }

.cdra__section { display: flex; flex-direction: column; gap: 0.45rem; }
.cdra__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
...
.cdra__day { display: flex; flex-direction: column; align-items: center; gap: 0.2rem; }
.cdra__stack {
  flex: 1;
  width: 100%;
...
.cdra__inb { background: var(--accent); }
.cdra__out { background: var(--ink); }

.cdra__day-label {
  font-family: var(--mono); font-size: 0.62rem; color: var(--muted); letter-spacing: 0.08em;
}
.cdra__day-sum {
  font-family: var(--mono); font-size: 0.72rem; color: var(--ink); font-weight: 700;
...
.cdra__legend span { display: inline-flex; align-items: center; gap: 0.35rem; }
.cdra__sw { width: 10px; height: 10px; display: inline-block; border-radius: 2px; }
.cdra__sw--inb { background: var(--accent); }
.cdra__sw--out { background: var(--ink); }

.cdra__dests { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.3rem; }
.cdra__dest {
  display: grid;
...
.cdra__name { font-size: 0.88rem; font-weight: 500; }
.cdra__prefix {
...
.cdra__meter {
  background: var(--paper-deep); border-radius: 2px; height: 8px;
  overflow: hidden;
}
.cdra__meter-fill { display: block; height: 100%; background: var(--accent); }
.cdra__calls {
