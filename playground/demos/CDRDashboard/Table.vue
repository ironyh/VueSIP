  ...
  <div class="cdrt">
    <header class="cdrt__head">
      <div>
        ...
        <h3 class="cdrt__title">
          {{ filtered.length }} / {{ rows.length }} rows · {{ totalMin }} min billed
        </h3>
      </div>
      <div class="cdrt__filters">
        <input
          type="search"
          v-model="query"
          placeholder="Search uri, number, disposition…"
          aria-label="Search CDR"
          class="cdrt__search"
        />
      ...
      <table class="cdrt__table">
        ...
        <tbody>
          <tr v-for="r in filtered" :key="r.id" :class="`cdrt__row cdrt__row--${r.disp}`">
            ...
            <td>
              <code class="cdrt__uri">{{ r.to }}</code>
            </td>
            <td class="cdrt__num">{{ fmtDur(r.duration) }}</td>
            <td>
              <span class="cdrt__disp">{{ r.disp }}</span>
            </td>
    ...
    <footer class="cdrt__foot">
      <span
        >Source: Asterisk `cdr_csv` → Postgres. Cost computed post-hangup against the carrier rate
        deck; billable duration excludes ring-time.</span
      >
    </footer>
  </div>
</template>
