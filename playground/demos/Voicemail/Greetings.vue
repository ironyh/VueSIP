      ...
      <ul class="grt__states" role="list">
        ...
        >
          ...
          <div class="grt__state-tools">
            ...
            >
              <span class="grt__record-dot" aria-hidden="true"></span>
            ...
            <button
              ...
              @click="activeId = g.id"
            >
              {{ activeId === g.id ? 'Default' : 'Set default' }}
            </button>
          </div>
        </li>
      </ul>
...
const greetings = ref<Greeting[]>([
  {
    ...
    script:
      "Hi, you've reached Priya. I'm unable to take your call — leave a message with your name and number and I'll get back to you.",
    recorded: true,
    duration: 14,
  },
  {
...
const fallbacks = [
  {
    ...
    desc: 'Caller records; transcribed to inbox (default).',
  },
  {
    id: 'announcement' as const,
    label: 'Announcement only',
    desc: 'Plays greeting, disconnects — no message recorded.',
  },
  {
    id: 'forward' as const,
    label: 'Forward to mobile',
    desc: 'Bridges to +1-415-555-0199 after greeting.',
  },
  {
    id: 'ring' as const,
    label: 'Keep ringing',
    desc: 'No voicemail at all — 180 Ringing indefinitely.',
  },
]
...
const dialPreview = computed(() =>
  [
    ...
    greetings.value
      .filter((g) => g.recorded && g.id !== 'default')
      .map((g) => `  same => n,ExecIf($[${g.trigger}]?Playback(greet-${g.id}))`)
      .join('\n'),
    `  same => n,Playback(greet-${activeId.value})`,
    fallback.value === 'voicemail'
      ? '  same => n,VoiceMail(priya@default,u)  ; "u" = unavail, leave message'
      : fallback.value === 'announcement'
        ? '  same => n,Hangup()'
        : fallback.value === 'forward'
          ? '  same => n,Dial(PJSIP/+14155550199,30,g)'
...
.grt__record--on .grt__record-dot {
  animation: grt-blink 0.9s steps(2, start) infinite;
}
@keyframes grt-blink {
  50% {
    opacity: 0.2;
...
.grt__fallback-btn:hover {
  border-color: var(--ink);
}
.grt__fallback-btn--on {
  background: var(--ink);
  color: var(--paper);
  border-color: var(--ink);
}
.grt__fallback-btn--on .grt__fallback-desc {
