<template>
  <div class="cql">
    <header class="cql__head">
      ...
      <span class="cql__dot" :class="`cql__dot--${verdictTier}`" aria-hidden="true"></span>
    </header>
    ...
    <footer class="cql__foot">
      <span
        >Pulled from <code>peerConnection.getStats()</code> — `outbound-rtp`, `remote-inbound-rtp`,
        `candidate-pair`. Refreshed every second.</span
      >
    </footer>
  </div>
</template>
...
import { calculateMos, getQualityVerdict, metricTier } from './qualityModel'
...
const mos = computed(() => calculateMos({ jitter: jitter.value, loss: loss.value, rtt: rtt.value }))
...
const metrics = computed<M[]>(() => [
  {
    key: 'jitter',
    label: 'Jitter',
    value: jitter.value.toFixed(1),
    unit: 'ms',
    tier: metricTier(jitter.value, 30, 60),
  },
  {
    key: 'loss',
    label: 'Packet loss',
    value: loss.value.toFixed(2),
    unit: '%',
    tier: metricTier(loss.value, 1, 3),
  },
  {
    key: 'rtt',
    label: 'Round-trip',
    value: rtt.value.toFixed(0),
    unit: 'ms',
    tier: metricTier(rtt.value, 150, 300),
  },
