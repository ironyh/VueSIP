<template>
  <div class="cqs">
    ...
    <footer class="cqs__foot">
      <span>Recap pulled post-hangup from stored `getStats()` samples — written alongside the CDR row so ops can correlate with Asterisk's log.</span>
    </footer>
  </div>
</template>

<script setup lang="ts">
interface Event { t: number; kind: string; tier: 'ok' | 'warn' | 'bad'; msg: string }
...
const call = {
  ...
  events: [
    ...
    { t: 104, kind: 'QUAL',      tier: 'warn', msg: 'RTT spike to 280 ms · jitter buffer grew to 120 ms' },
...
.cqs__head { display: flex; justify-content: space-between; align-items: center; gap: 0.75rem; flex-wrap: wrap; }
.cqs__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.cqs__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
.cqs__verdict {
  font-family: var(--mono); font-size: 0.68rem; font-weight: 700;
  letter-spacing: 0.08em; text-transform: uppercase;
  padding: 0.3rem 0.6rem; border-radius: 2px;
}
.cqs__verdict--ok { background: color-mix(in srgb, #15803d 15%, transparent); color: #15803d; }
.cqs__verdict--warn { background: color-mix(in srgb, #d97706 15%, transparent); color: #d97706; }
.cqs__verdict--bad { background: color-mix(in srgb, #b91c1c 15%, transparent); color: #b91c1c; }
...
.cqs__u { font-family: var(--mono); font-size: 0.62rem; color: var(--muted); }

.cqs__section { display: flex; flex-direction: column; gap: 0.4rem; }
.cqs__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}
.cqs__events { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.25rem; }
.cqs__event {
  display: grid;
...
.cqs__badge--warn { color: #d97706; border-color: #d97706; }
.cqs__badge--bad { color: #b91c1c; border-color: #b91c1c; }
.cqs__msg { font-size: 0.82rem; color: var(--ink); }
