...
export const metricTier = (value: number, warn: number, bad: number): QualityTier =>
  value >= bad ? 'bad' : value >= warn ? 'warn' : 'ok'
...
export const getNetworkBanner = (
  ...
  if (snapshot.loss >= 3 || snapshot.rtt >= 300 || snapshot.jitter >= 60) {
    return {
      ...
      detail:
        'Show a persistent danger banner, freeze non-essential animations, and offer audio-only fallback.',
    }
  }
  ...
  if (snapshot.loss >= 1 || snapshot.rtt >= 150 || snapshot.jitter >= 30) {
    return {
      ...
      detail:
        'Warn the user early, keep controls visible, and suggest moving to a stronger connection before speech becomes robotic.',
    }
  }
