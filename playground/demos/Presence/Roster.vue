    ...
    <ul class="roster__groups" role="list">
      <li v-for="group in groups" :key="group.label" class="roster__group">
        ...
        <ul class="roster__list" role="list">
          <li v-for="m in group.members" :key="m.id" class="roster__row" :class="`roster__row--${m.state}`">
            <span class="roster__avatar" aria-hidden="true">
              {{ initials(m.name) }}
              <span class="roster__dot" />
            </span>
            ...
            <div class="roster__tools">
              <button
                ...
                :title="m.state === 'dnd' ? 'Do-Not-Disturb' : m.state === 'offline' ? 'Offline' : 'Call'"
...
const members = ref<Member[]>([
  { id: 1, name: 'Alex Rivera', uri: 'sip:alex@example.com', state: 'available', team: 'Engineering' },
  { id: 2, name: 'Priya Shah', uri: 'sip:priya@example.com', state: 'busy', note: 'In a call', team: 'Engineering' },
  { id: 3, name: 'Mei Chen', uri: 'sip:mei@example.com', state: 'away', note: 'Back in 20 min', team: 'Engineering' },
  { id: 4, name: 'Jordan Lee', uri: 'sip:jordan@example.com', state: 'dnd', note: 'Deep work until 3 PM', team: 'Design' },
  ...
  { id: 7, name: 'Riley Kim', uri: 'sip:riley@example.com', state: 'busy', note: 'On customer call', team: 'Support' },
...
const availableCount = computed(() =>
  members.value.filter((m) => m.state === 'available').length
)
...
const stateOrder: Record<State, number> = {
  available: 0, busy: 1, away: 2, dnd: 3, offline: 4,
}
...
const groups = computed(() => {
  ...
  if (grouping.value === 'none') {
    ...
    return [{
      label: 'All',
      members: list.sort((a, b) => stateOrder[a.state] - stateOrder[b.state] || a.name.localeCompare(b.name)),
    }]
  }
  ...
  return entries.map(([label, members]) => ({
    label,
    members: members.sort((a, b) => stateOrder[a.state] - stateOrder[b.state] || a.name.localeCompare(b.name)),
  }))
})
...
.roster__sort-btn:hover { color: var(--ink); }
.roster__sort-btn--on { background: var(--ink); color: var(--paper); }
...
.roster__search-input:focus { outline: none; border-color: var(--accent); }
.roster__toggle {
  display: inline-flex;
  align-items: center;
  gap: 0.4rem;
...
.roster__toggle input { accent-color: var(--accent); }

.roster__groups { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.65rem; }
...
.roster__list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.3rem; }
.roster__row {
  display: grid;
...
.roster__row:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.roster__row--offline { opacity: 0.6; }
...
.roster__row--busy .roster__dot { background: var(--c-busy); }
.roster__row--away .roster__dot { background: var(--c-away); }
.roster__row--dnd .roster__dot { background: var(--c-dnd); }
.roster__row--offline .roster__dot { background: var(--c-offline); }
...
.roster__name { font-weight: 600; font-size: 0.9rem; }
.roster__state-chip {
  font-family: var(--mono);
  font-size: 0.6rem;
...
.roster__row--available .roster__state-chip { color: var(--c-available); }
.roster__row--busy .roster__state-chip { color: var(--c-busy); }
.roster__row--away .roster__state-chip { color: var(--c-away); }
.roster__row--dnd .roster__state-chip { color: var(--c-dnd); }
...
.roster__dot-sep { opacity: 0.6; }
.roster__note { font-family: var(--sans); }

.roster__tools { display: inline-flex; gap: 0.2rem; }
.roster__tool {
  width: 28px;
  height: 28px;
  background: transparent;
...
.roster__tool:hover:not(:disabled) { color: var(--accent); border-color: var(--accent); }
.roster__tool:disabled { opacity: 0.4; cursor: not-allowed; }
