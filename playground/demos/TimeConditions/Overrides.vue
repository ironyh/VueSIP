<template>
  <div class="tco">
    <header class="tco__head">
      ...
      <span class="tco__dot" :class="`tco__dot--${override.mode}`" aria-hidden="true" />
    </header>
    ...
    <footer class="tco__foot">
      <span>Override flips the <code>ASTDB(timecondition/support)</code> key; dialplan reads it before evaluating <code>GotoIfTime()</code>. Survives <code>asterisk -rx "core restart"</code>.</span>
    </footer>
  </div>
</template>
...
const durations = [
  { label: '1 hour' }, { label: '2 hours' }, { label: 'Rest of day' },
  { label: 'Until Mon 09:00' }, { label: 'Indefinite' },
]
...
const history = [
  ...
  { at: 'Mar 08 08:47', by: 'priya@ops',      mode: 'open' as Mode,   reason: 'Early-bird launch support' },
  { at: 'Feb 29 16:15', by: 'jordan@support', mode: 'closed' as Mode, reason: 'Snow day — east coast' },
...
.tco__head { display: flex; justify-content: space-between; align-items: center; gap: 0.75rem; flex-wrap: wrap; }
.tco__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.tco__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; letter-spacing: 0.04em; }

.tco__dot { width: 12px; height: 12px; border-radius: 50%; }
.tco__dot--auto { background: var(--muted); }
.tco__dot--open { background: #15803d; }
.tco__dot--closed { background: #b91c1c; }
...
.tco__mode:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.tco__mode--on { border-color: var(--accent); background: color-mix(in srgb, var(--accent) 6%, transparent); }
.tco__mode-label { font-weight: 600; font-size: 0.9rem; }
.tco__mode-desc { font-family: var(--mono); font-size: 0.66rem; color: var(--muted); }

.tco__section { display: flex; flex-direction: column; gap: 0.35rem; }
.tco__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}
.tco__durations { list-style: none; padding: 0; margin: 0; display: flex; gap: 0.3rem; flex-wrap: wrap; }
.tco__dur {
  background: var(--paper); border: 1px solid var(--rule); border-radius: 2px;
  padding: 0.4rem 0.8rem;
  font-family: var(--mono); font-size: 0.72rem; color: var(--muted); cursor: pointer;
}
.tco__dur--on {
  color: var(--paper); background: var(--accent); border-color: var(--accent);
}

.tco__log { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.25rem; }
.tco__log-row {
  display: grid;
...
@media (max-width: 600px) { .tco__log-row { grid-template-columns: 1fr; } }
.tco__log-at {
  font-family: var(--mono); font-size: 0.72rem; color: var(--muted);
  font-variant-numeric: tabular-nums;
}
.tco__log-by { font-family: var(--mono); font-size: 0.74rem; color: var(--ink); }
.tco__log-mode {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
...
.tco__log-mode--open { color: #15803d; border-color: #15803d; }
.tco__log-mode--closed { color: #b91c1c; border-color: #b91c1c; }
.tco__log-reason { font-size: 0.82rem; color: var(--ink); }
