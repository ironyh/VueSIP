<template>
  <div class="tcs">
    <header class="tcs__head">
      ...
      <span class="tcs__now" :class="`tcs__now--${currentState}`">Now: {{ currentState.toUpperCase() }}</span>
    </header>

    <div class="tcs__grid" role="grid" aria-label="Weekly business hours">
      <div class="tcs__hcorner" />
    ...
    <footer class="tcs__foot">
      <span>Asterisk: <code>GotoIfTime(09:00-18:00|mon-fri|*|*?open,s,1)</code>. Drop timezone with <code>TIMEZONE(office)</code> if your server is UTC.</span>
    </footer>
  </div>
</template>
...
interface Day { name: string; hours: number[] }
...
.tcs__head { display: flex; justify-content: space-between; align-items: flex-end; gap: 0.75rem; flex-wrap: wrap; }
.tcs__eyebrow {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted);
}
.tcs__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; font-variant-numeric: tabular-nums; }
...
.tcs__now--open { background: color-mix(in srgb, #15803d 15%, transparent); color: #15803d; }
.tcs__now--closed { background: color-mix(in srgb, #b91c1c 15%, transparent); color: #b91c1c; }
...
.tcs__cell:hover { border-color: var(--accent); }
.tcs__cell--on {
  background: var(--accent);
  border-color: var(--accent);
}

.tcs__section { display: flex; flex-direction: column; gap: 0.35rem; }
.tcs__section-title {
  font-family: var(--mono); font-size: 0.64rem; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted);
}
.tcs__holidays { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.25rem; }
.tcs__holiday {
...
.tcs__hdate {
  font-family: var(--mono); font-size: 0.78rem; font-weight: 700; color: var(--accent);
  font-variant-numeric: tabular-nums;
}
.tcs__hname { font-size: 0.88rem; }
.tcs__hrule {
