<template>
  <div class="directory">
    <div v-if="!isConnected" class="directory__status">
      <span class="directory__status-led" aria-hidden="true"></span>
        ...
        <ul class="directory__list">
          <li v-for="(c, i) in contacts" :key="c.number">
            <button
              ...
              @click="dial(c.number)"
            >
              <span class="directory__contact-index" aria-hidden="true">{{
                String(i + 1).padStart(2, '0')
              }}</span>
              <span :class="['directory__avatar', { 'is-icon': !!c.icon }]" aria-hidden="true">{{
    ...
    <section v-else-if="isIncoming" class="directory__panel" aria-live="polite" aria-atomic="true">
      <div class="directory__live-head">
        <span class="directory__pulse" aria-hidden="true"></span>
      ...
      <div class="directory__actions">
        <button
          type="button"
          class="directory__cta directory__cta--go"
          @click="answer({ audio: true, video: false })"
        >
          Answer
        </button>
        <button type="button" class="directory__cta directory__cta--stop" @click="reject(486)">
          Reject
        </button>
      </div>
...
.directory__status-label {
  color: var(--muted);
}
.directory__status-value {
  color: var(--ink);
  font-weight: 700;
}
