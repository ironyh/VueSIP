    ...
    <div class="rib__preview">
      ...
      <div class="rib__frame">
        <div class="rib__frame-head">
          <span class="rib__frame-dot"></span>
          <span class="rib__frame-dot"></span>
          <span class="rib__frame-dot"></span>
      ...
      <table class="rib__table">
        ...
        <tbody>
          <tr
            v-for="r in regions"
            :key="r.region"
            :class="{ 'rib__row--active': r.region === jurisdiction }"
          >
            <td>
              <code>{{ r.region }}</code>
            </td>
...
const jurisdictions = [
  {
    id: 'US-1P' as const,
    label: 'US (1-party)',
    desc: '38 states + federal — only caller needs to consent',
  },
  {
    id: 'US-2P' as const,
    label: 'US (2-party)',
    desc: 'CA, FL, IL, PA, WA + 7 others — both sides must consent',
  },
  {
    id: 'EU' as const,
    label: 'EU (GDPR)',
    desc: 'Explicit lawful basis required; data-subject rights apply',
  },
  ...
  { id: 'AU' as const, label: 'AU', desc: 'Telecommunications Interception Act; beep accepted' },
  {
    id: 'JP' as const,
    label: 'JP',
    desc: 'APPI — notice-based; explicit consent for sensitive data',
  },
]
  ...
  switch (jurisdiction.value) {
    case 'US-1P':
      return {
        style: 'subtle',
        icon: '◉',
        title: 'Call is being recorded',
        text: 'Recording is logged for quality review.',
        ref: '18 U.S.C. § 2511',
      }
    case 'US-2P':
      return {
        ...
        text: 'By staying on the line you consent to recording for quality and training. Say "stop recording" to opt out.',
        ref: 'CA Penal § 632',
      }
    case 'EU':
      return {
        style: 'modal',
        icon: '🛡',
        title: 'Recording with GDPR notice',
        text: 'Lawful basis: legitimate interest (quality assurance). Retention 30 days. You may request deletion via privacy@example.com.',
        ref: 'GDPR Art. 6(1)(f)',
      }
    case 'UK':
      return {
        style: 'prominent',
        icon: '🔔',
        title: 'Recorded — periodic beep enabled',
        text: 'You will hear a short tone every 15 seconds throughout the call.',
        ref: 'ICO RIPA',
      }
    case 'AU':
      return {
        style: 'prominent',
        icon: '🔔',
        title: 'Call recorded — tone present',
        text: 'A recording indicator tone is played periodically in compliance with the TIA.',
        ref: 'TIA 1979',
      }
    case 'JP':
      return {
...
.rib__frame-dot:nth-child(1) {
  background: #e06c4a;
}
.rib__frame-dot:nth-child(2) {
  background: #e2b437;
}
.rib__frame-dot:nth-child(3) {
  background: #6ca97b;
}
.rib__frame-url {
  margin-left: 0.5rem;
