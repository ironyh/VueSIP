  ...
  <div class="blf">
    <header class="blf__head">
      ...
      <div class="blf__legend" role="list" aria-label="Lamp legend">
        <span class="blf__legend-item" role="listitem">
          <span class="blf__lamp blf__lamp--idle" aria-hidden="true" />Idle
        </span>
        <span class="blf__legend-item" role="listitem">
          <span class="blf__lamp blf__lamp--ringing" aria-hidden="true" />Ringing
        </span>
        <span class="blf__legend-item" role="listitem">
          <span class="blf__lamp blf__lamp--busy" aria-hidden="true" />Busy
        </span>
        <span class="blf__legend-item" role="listitem">
          <span class="blf__lamp blf__lamp--dnd" aria-hidden="true" />DND
        </span>
        <span class="blf__legend-item" role="listitem">
          <span class="blf__lamp blf__lamp--offline" aria-hidden="true" />Offline
        </span>
      </div>
    </header>
      ...
      <li v-for="e in extensions" :key="e.ext" class="blf__cell">
        ...
        >
          <span class="blf__lamp-row">
            <span class="blf__lamp" :class="`blf__lamp--${e.state}`" aria-hidden="true" />
          ...
          <span class="blf__peer">
            <template v-if="e.state === 'busy' && e.peer">
              ← {{ e.peer }}
            </template>
            <template v-else-if="e.state === 'ringing' && e.peer">
              ⇽ {{ e.peer }}
            </template>
            <template v-else-if="e.state === 'dnd'">
              Do not disturb
            </template>
            <template v-else-if="e.state === 'offline'">
              Unregistered
            </template>
            <template v-else>
              &nbsp;
    ...
    <footer class="blf__foot">
      <span class="blf__source">
        <code>SUBSCRIBE</code> · event <code>dialog</code> · accept <code>application/dialog-info+xml</code>
      </span>
...
const extensions = ref<Ext[]>([
  { ext: '2001', name: 'Alex Rivera', state: 'busy', peer: '+14155550100', dur: 147, since: Date.now() },
  { ext: '2002', name: 'Priya Shah', state: 'idle', dur: 0, since: Date.now() },
  { ext: '2003', name: 'Tomás León', state: 'ringing', peer: '+442079460000', dur: 0, since: Date.now() },
  { ext: '2004', name: 'Jun Okafor', state: 'busy', peer: 'sip:bridge@conf.local', dur: 1842, since: Date.now() },
  { ext: '2005', name: 'Sara Kovač', state: 'dnd', dur: 0, since: Date.now() },
  { ext: '2006', name: 'Lee Park', state: 'idle', dur: 0, since: Date.now() },
  { ext: '2007', name: 'Mika Weiss', state: 'busy', peer: '+33142688000', dur: 23, since: Date.now() },
  { ext: '2008', name: 'Noor Hassan', state: 'offline', dur: 0, since: Date.now() },
  { ext: '2009', name: 'Ravi Menon', state: 'idle', dur: 0, since: Date.now() },
  { ext: '2010', name: 'Greta Møller', state: 'ringing', peer: '+4560111222', dur: 0, since: Date.now() },
...
const stateLabel = (s: State) =>
  s === 'idle' ? 'Idle' : s === 'busy' ? 'On call' : s === 'ringing' ? 'Ringing' : s === 'dnd' ? 'DND' : 'Offline'
...
.blf__pill--idle { color: var(--idle); border-color: color-mix(in srgb, var(--idle) 40%, var(--rule)); }
.blf__pill--busy { color: var(--busy); border-color: color-mix(in srgb, var(--busy) 40%, var(--rule)); }
.blf__pill--ringing { color: var(--ringing); border-color: color-mix(in srgb, var(--ringing) 40%, var(--rule)); }
...
.blf__tile:hover { border-color: var(--ink); }
.blf__tile--idle { border-left-color: var(--idle); }
.blf__tile--busy {
...
.blf__tile--dnd { border-left-color: var(--dnd); }
.blf__tile--offline { border-left-color: var(--offline); opacity: 0.75; }

@keyframes blfPulse {
  0%, 100% { box-shadow: 0 0 0 0 color-mix(in srgb, var(--ringing) 40%, transparent); }
  50% { box-shadow: 0 0 0 4px color-mix(in srgb, var(--ringing) 10%, transparent); }
}
...
.blf__lamp--idle { background: var(--idle); box-shadow: 0 0 6px color-mix(in srgb, var(--idle) 70%, transparent); }
.blf__lamp--busy { background: var(--busy); box-shadow: 0 0 6px color-mix(in srgb, var(--busy) 70%, transparent); }
.blf__lamp--ringing { background: var(--ringing); box-shadow: 0 0 8px color-mix(in srgb, var(--ringing) 90%, transparent); }
.blf__lamp--dnd { background: var(--dnd); }
.blf__lamp--offline { background: var(--offline); }
...
.blf__tick { font-variant-numeric: tabular-nums; }
</style>
