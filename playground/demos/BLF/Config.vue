<template>
  <div class="blfc">
        ...
        <div class="blfc__row-main">
          <div class="blfc__row-head">
            ...
            <span class="blfc__status">
              <span class="blfc__dot" :class="`blfc__dot--${w.subStatus}`" aria-hidden="true" />
    ...
    <section class="blfc__globals">
      ...
      <label class="blfc__global">
        ...
        <span>
          ...
          <span class="blfc__hint">One dialog, many extensions. Cuts PBX load 10–50×; requires compatible server.</span>
        </span>
      </label>
      <label class="blfc__global">
        ...
        <span>
          ...
          <span class="blfc__hint">Standard practice — without it a lost <code>NOTIFY</code> leaves stale lamps.</span>
        </span>
      </label>
      <label class="blfc__global">
        ...
        <span>
          ...
          <span class="blfc__hint">Asterisk <code>pickupgroup=</code> / FreeSWITCH <code>pickup_group</code>.</span>
        </span>
      </label>
    </section>
    ...
    <footer class="blfc__foot">
      <span>
        Template ·
        <code>{{ subscribeTemplate }}</code> ·
        <code>Event: dialog</code> ·
...
const watches = ref<Watch[]>([
  { ext: '2001', label: 'Alex Rivera · sales', action: 'call', favorite: true, subStatus: 'active', expires: 3600, renewedAgo: 142 },
  { ext: '2002', label: 'Priya Shah · sales', action: 'call', favorite: false, subStatus: 'active', expires: 3600, renewedAgo: 87 },
  { ext: '2003', label: 'Tomás León · sales', action: 'pickup', favorite: false, subStatus: 'active', expires: 3600, renewedAgo: 54 },
  { ext: '4000', label: 'Queue · support', action: 'barge', favorite: true, subStatus: 'active', expires: 1800, renewedAgo: 301 },
  { ext: '9001', label: 'Paging · all floors', action: 'intercom', favorite: false, subStatus: 'pending', expires: 3600, renewedAgo: 0 },
  { ext: 'sip:alex@example.com', label: 'Alex · mobile AOR', action: 'transfer', favorite: false, subStatus: 'failed', expires: 3600, renewedAgo: 0 },
])
...
.blfc__title { margin: 0.1rem 0 0; font-size: 1rem; font-weight: 600; }
...
.blfc__add:hover { background: var(--accent); }
...
@media (max-width: 720px) { .blfc__form-row { grid-template-columns: 1fr; } }
.blfc__field { display: flex; flex-direction: column; gap: 0.25rem; }
.blfc__label {
  font-family: var(--mono);
  font-size: 0.62rem;
  font-weight: 700;
...
.blfc__input:focus { outline: none; border-color: var(--accent); }
.blfc__form-actions { display: flex; justify-content: flex-end; gap: 0.4rem; }
.blfc__cancel, .blfc__save {
  border-radius: 2px;
...
.blfc__cancel { background: transparent; color: var(--muted); border: 1px solid var(--rule); }
.blfc__cancel:hover { color: var(--ink); border-color: var(--ink); }
.blfc__save { background: var(--accent); color: var(--paper); border: 0; }
...
.blfc__row:hover { border-color: color-mix(in srgb, var(--accent) 40%, var(--rule)); }
.blfc__row-main { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 0.2rem; }
.blfc__row-head {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
...
.blfc__dot--active { background: var(--ok); }
.blfc__dot--pending { background: var(--warn); }
.blfc__dot--failed { background: var(--err); }
...
.blfc__sep { opacity: 0.5; }

.blfc__tools { display: inline-flex; gap: 0.25rem; flex-shrink: 0; }
.blfc__tool {
...
.blfc__tool:hover { color: var(--ink); border-color: var(--ink); }
.blfc__tool--on {
  color: var(--accent);
  border-color: var(--accent);
  background: color-mix(in srgb, var(--accent) 10%, transparent);
}
.blfc__tool--danger { font-size: 1rem; line-height: 1; padding: 0.2rem 0.55rem; }
.blfc__tool--danger:hover { color: var(--err); border-color: var(--err); }
...
.blfc__global input { accent-color: var(--accent); margin-top: 0.3rem; }
.blfc__global code {
  font-family: var(--mono);
  background: var(--paper);
  border: 1px solid var(--rule);
