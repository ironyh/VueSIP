<template>
  <div class="connection-manager" data-testid="connection-manager-panel">
    ...
    <div v-if="isCurrentlyConnected && activeConnectionInfo" class="active-connection-panel">
      ...
      <button class="btn btn-danger btn-disconnect" @click="handleDisconnect">Disconnect</button>
    </div>
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
        <div class="modal-content">
          ...
          <form @submit.prevent="saveConnection" class="modal-body">
            <div class="sandbox-preset" v-if="!isEditing" data-testid="sandbox-preset">
              <div class="sandbox-preset__copy">
                <strong>No PBX handy?</strong>
                Use the free shared sandbox. No outbound PSTN, no privacy — dial
                <code>600</code> for an echo test, or <code>demo2</code>..<code>demo6</code> to call
                another sandbox user.
              </div>
              <button
                type="button"
                class="btn btn-sm sandbox-preset__btn"
                data-testid="load-sandbox-preset"
...
function loadSandboxPreset() {
  ...
  const hostPort = isLocalHost ? 'localhost:18089' : 'pbx-demo.vuesip.com'
