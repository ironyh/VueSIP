...
- WS on `:8088`, WSS on `:8089` (self-signed cert generated on first boot)
- AMI on `:5038` (user `sandbox`, password `sandbox-ami-demo`) for the
  call-center demo
- Dialplan targets:
  - `600` — Echo test
...
`https://localhost:8089/httpstatus` once and accept the cert.
