...
export function validateInterface(interfaceName: string): ValidationResult {
  ...
  if (!interfaceName) {
    ...
    errors.push('Interface must be in format: PROTOCOL/ENDPOINT (e.g., PJSIP/1001)')
  }
 ...
 * ```
 */
export function calculatePercentage(numerator: number, denominator: number): PercentageMetric {
  if (denominator === 0) {
...
export function calculateRate(
  ...
  const rate =
    durationSeconds > 0 ? Math.round((count / durationSeconds) * divisors[unit] * 100) / 100 : 0
 ...
 * ```
 */
export function formatAmiDuration(seconds: number, format: 'short' | 'long' = 'short'): string {
 ...
 * ```
 */
export function safeParseNumber(value: string | number | undefined, fallback = 0): number {
 ...
 * ```
 */
export function createLookupMap<T>(items: T[], keyField: keyof T): Map<string, T> {
