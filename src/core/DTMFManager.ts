...
export class DTMFManager {
  ...
  private detectCapabilities(): void {
    ...
    if (connection) {
      ...
      if (localDescription && remoteDescription) {
        ...
        rfc2833Enabled =
          localSdp.includes('telephone-event') &&
          remoteSdp.includes('telephone-event');
      }
    }
    ...
    const preferredMethod: DTMFMethod = rfc2833Enabled
      ? 'rfc2833'
      : 'sipinfo';
  ...
  async sendDTMF(
    ...
    if (
      this.capabilities &&
      !this.capabilities.supportedMethods.includes(fullOptions.method)
    ) {
      // Try fallback
  ...
  private async sendSIPInfo(tone: DTMFTone, duration: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.session!.sendInfo(DTMF_CONSTANTS.SIPINFO_CONTENT_TYPE, body, options);
