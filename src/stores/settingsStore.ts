...
export const useSettingsStore = defineStore('settings', () => {
  ...
  function validateSettings(): SettingsValidationError[] {
    ...
    if (
      settings.value.network.stunServers.length === 0 &&
    ...
    if (settings.value.ami.enabled) {
      if (!settings.value.ami.url) {
        ...
        !settings.value.ami.url.startsWith('ws://') &&
        !settings.value.ami.url.startsWith('wss://')
      ) {
        errors.push({
          field: 'ami.url',
      ...
      if (
        settings.value.ami.connectionTimeout < 1000 ||
        settings.value.ami.connectionTimeout > 30000
      ) {
        errors.push({
